...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 10 PREFIX order:  # generation from FEED.TAIL: an identity, not a counter
```
...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 100 PREFIX acct:  # who set what, in commit order
```
...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 10 PREFIX user:  # the mirror consumer's read loop
...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 100 PREFIX order:  # the CDC-to-warehouse read loop
```
...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 100 PREFIX session:  # generation from FEED.TAIL: an identity, not a counter
```
...
kevy-cli -p 6004 FEED.READ 0 $(kevy-cli -p 6004 FEED.TAIL 0 | head -1) 0 COUNT 100 PREFIX reading:  # the uplink loop
```
