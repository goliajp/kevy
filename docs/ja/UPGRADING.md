...
**6.2 から来た方へ。** そのホップはこのページではなく [upgrading-6.2-to-6.3.md](upgrading-6.2-to-6.3.md) です。
...
`kevy-client`と`kevy-client-async`はワークスペースのバージョン線に**乗っていません**。出荷されるのは**2.0.0**であり、4.0.0ではありません。`cargo add kevy-client`が2.xを解決するのは、それが最新版だからです（古い版ではありません）。（6.2.1 まではそうでした。以降、両クライアントはワークスペースのバージョンに揃い、`"2"` の要求は 5.x エンジンに対して組まれたクライアントに解決されます——[upgrading-6.2.0-to-6.2.1.md](upgrading-6.2.0-to-6.2.1.md) を参照。）
...
**AOFに関する注意がひとつ：レガシーな`SPOP`フレーム。** 4.0はSPOPを本当にランダムにし、したがって動詞ではなく*その効果*——`SREM key <popped…>`、実際に取り除かれたメンバー——をログし（そしてレプリケートし）ます。一度もリライトされていない3.x時代のAOFは、生の`SPOP`フレームをまだ抱えている可能性があります。それを4.0でリプレイすると新しいランダムな選択が引かれるため、その最初のリプレイ後に残る集合は、アップグレード前のプロセスのものと違いうるのです。集合にSPOPのトラフィックがあるなら、アップグレードの直前か直後に一度`BGREWRITEAOF`を走らせて現在の状態を実体化してください。それ以降、ログは決定的なフレームだけを運びます。
...
明示的に残した例外がひとつ：CDCフィードのembedded面（`changes_tail` / `changes_since`）は自分の`FeedError`を保ちます——`Resync`と`Future`は、このストリーム固有の制御信号であって、汎用のエラーではありません。それらを汎用のエラー列挙に畳み込めば、呼び手が唯一必ず扱わねばならないものを`?`で通り過ぎるよう誘うことになります。[CDCフィード](cdc.md)を参照してください。
...
## 階層化と TABLE 層（4.0 での新顔、すべて加算的）

4.0 は透過的な階層化（[tiering.md](tiering.md)）と `TABLE.*` 層（[tables.md](tables.md)）も同梱します。どちらもアップグレードで自動的に有効になることはありません。この節があるのは、**足元で何が変わり、何が変わらなかったか**を知ってもらうためです。

- **階層化は opt-in——使わないなら操作はゼロ。** `[tiering]` セクションが無ければ（そして `with_tier_budget*` を呼ばなければ）今日の経路がバイト単位で保たれます。その「切っているときのコスト」自体が門で見張られている主張です。有効にしても永続性の契約は変わりません：AOF は唯一の永続的な真実のままで、`<data>/tier/` の下の冷たい値ログは**起動ごとに使い捨て**です（開くたびに削除され、バックアップされません）。
- **`INFO` に `# Tiering` セクションが増える——階層化が有効なときだけ。** 階層化していない 4.0 サーバの `INFO` 出力は変わらないので、セクションを列挙するパーサは、あなたが自分で有効にするまで何も新しいものを見ません。
- **新しい動詞、すべて加算的**：`TABLE.DECLARE` / `TABLE.DROP` / `TABLE.LIST` / `TABLE.VERIFY`、`IDX.QUERY` / `IDX.COUNT` の `WHERE` 形（複合の先頭接頭辞による探索）、そしてスカラの句一式——`IDX.CREATE` 時の `VALUES`、クエリ時の `FILTER` / `SORT` / `DISTINCT` / `FACET` / `OFFSET`——は、以前はテキスト専用でしたが、いまは `range`/`unique` の kind でも使えます。**既存の動詞で形が変わったものはありません。**
- **新しいクレートが二つ**ワークスペースに加わります：`kevy-vlog`（冷たい値ログの石）と `kevy-sql`（エンジンの外にある宣言時の SQL コンパイラ、`kevy-cli sql` という顔つき）。組み込みには `tier` という cargo フィーチャが増えます（既定の集合に入っています——上の表を参照）。
- **サイドカーのカタログ版 v5/v6。** インデックスカタログのサイドカーに v5（スカラ kind が保存する `VALUES`）と v6（複合 ORDERPATH インデックス）が加わり、新たに `table-catalog.meta` サイドカーができました。**それより古いサイドカー版もすべて読み込めます**——最初のインデックス版が残した v1 ファイルを含めて。書き手が出すのは**そのデータを表現できる最も古いヘッダ**です：新しい能力を一つも使っていないカタログは、これまでの版をバイト単位でそのまま書き続けるので、より前の 4.0 系のバイナリからも読めるままです。版が前に進むのは、あなたが実際に新しいものを宣言したときだけです。
...
- **3.8.0** —— 性能のアーク（valkey 9.1と、redis-stack 7.4.7のRediSearchに対する実測。素の面で1.6〜3.3倍、ANNは再現率1.000で1.64倍の先行、FTSの単一の一般的な語で93倍。組み込みをプライマリにするレプリケーション）。3.0.0と3.8.0のあいだにリリースは切られていません。3.8.0はv3.1〜v3.8の列車を含みます。
