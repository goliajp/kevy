...
具体的には、クロージャから`Err`を返すと、それが行ったすべての書き込みが——メモリ上でもAOF上でも——巻き戻されます。拒否されたトランザクションは痕跡を残さないので、検証がすべての書き込みより先に来るようにクロージャを組み立てる必要はありません。（この保証が本物になったのは4.0からです。それ以前のバージョンは、拒否されたクロージャの書き込みをメモリ上には生かしたままAOFフレームだけを捨てていたため、再起動が動作中のプロセスと食い違いました。）
