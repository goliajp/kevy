...
- **`VERIFY` が監査しないもの：集約のカウンタ。** `KIND agg` ではエントリは行ではなく**グループ**なので、「このエントリの行はまだこの値を導出するか」という問い自体が当てはまりません——カウントが何であれ `drift` と `missing` はゼロのままで、走行中の合計が実行時にキースペースと突き合わせて再計算されることもありません。構造自身の算術は走査による参照実装と単体テストで照合されていますが、キースペースとの一致は「すべての書き込み経路がそれを保守した」ことに依っており、この kind については `IDX.VERIFY` はそれを反証できません。代わりにテストが行います：`index_write_path_coverage` が各動詞のあとで、グループのカウントを実際に生きている行と突き合わせます。
