...
- **標準ライブラリは方言に従います。** 各方言はその方言が実際に持つ関数だけを受け取るので、ここで動くスクリプトは同じバージョンの本家 Lua でも動きます。既定の 5.1 では `table.setn` が存在し（PUC と同じく「'setn' is obsolete」を送出）、`table.unpack` / `table.create` は存在しません——5.1 の綴りはグローバルの `unpack` です。必要なら `#!lua version=5.4` でそれらを持つ方言を指定してください。luna-core 3.0.0 より前は、どの方言でも五つの版の和集合が見えており、対象バージョンにない名前をスクリプトが使えてしまっていました。
