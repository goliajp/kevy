...
両側ともハートビート/ACKに基づく真値を報告します（v3.14）。プライマリ側では、`slave0`行の`state`は、すべてのシャードストリームに本物の`REPLCONF ACK`が揃った時点で`syncing → online`に切り替わり、`offset`はその**acked**位置、`lag`はフレーム単位です。`ROLE`と`INFO replication`はシャードをまたいで集計します。オフセットはシャードストリーム全体で合算され、各レプリカプロセスはシャード数に関係なくちょうど1つの`slaveN`エントリとして現れます。レプリカ側では、直近3秒以内にハートビートが着地していれば`master_link_status`は`up`で、`slave_lag_frames:0`は追いついていることを意味します。フィールドごとの意味は[`docs/availability.md`](availability.md)の*Observability*節にあります。
...
| `min_replicas_max_lag_ms` | `10000` | `min_replicas_to_write`の鮮度ウィンドウ。最新のACKがこの上限より新しいレプリカだけが健全として数えられるので、止まってしまったレプリカは、接続が生きたままでもカウントから外れていきます。 |
...
何もしないでください。プライマリは、レプリカが要求したオフセットがもうバックログリングにないことを検出すると`TooOld`を返し、同じRESPワイヤ接続でシャードのキー空間スナップショットをインラインで送り、スナップショット末尾のオフセットからライブフレームを再開します。レプリカはスナップショットを差し替え、ライブの末尾を適用し、追いつきます。送出ウィンドウの間、レプリカはクライアント読み取りを`-LOADING`で拒否します（見えているキー空間がまるごと差し替えられようとしているからです）。`PING`、`INFO`、`HELLO`は免除され、`INFO replication`が`loading:1`を報告するので、監視はウィンドウが閉じるのを見張れます。空から再構築したければ、レプリカを止め、データディレクトリを削除して再起動してください。ランナーは`from_offset = 0`で接続し、キー空間全体がスナップショット送出されます。
