...
**このページの 2 つの測定は食い違っており、その食い違いは未解決である。** 順序の表の上にある AOF モードの表も「ロードしてから宣言」の順序であり、そちらは 13.5% **節約**する——こちらは 3.5% **かかる**。違うのは規模（200 万行と 50 万行）と、ベンチマーク側のテーブルはインデックスも宣言しており、そのバックフィルがパックのバックフィルの解放した直後に大量に確保する点である。アロケータが解放された表を抱え込まず再利用するなら説明がつくが、それはまだ誰も測っていない。−23.4% を表現そのものの効果として読み、2 つの「ロード先行」の数字は、バックフィル経路の結果がそのときプロセスが他に何をしているかに依存する、という意味に取ってほしい。
