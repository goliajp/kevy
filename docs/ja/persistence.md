...
ディレクトリ自体は、存在しなければ起動時に作成されます(v3.17)。作成できないパスは、最初に触れたサブシステムからの裸の ENOENT ではなく、名前の付いた 1 つの起動エラーになります。
...
| `elect.meta`(+ 一時的な `elect.meta.tmp`) | 選挙の耐久性(v3.15): エレクターの `(epoch, votedFor)` ペア。どの投票応答もノードを出る*前に*永続化されるため、クラッシュ再起動しても二重投票はあり得ない。tmp + fsync + rename で書かれる — 保存途中のクラッシュは旧ペアか新ペアのどちらかを残し、破れたファイルは決して残さない。`[cluster]` クォーラム構成時のみ存在。 |

## 耐久性契約(v2.1)

`appendfsync` × 書き込みパスごとに、「呼び出しが OK を返した」が何を保証するか。
"durable" = 安定ストレージ上にある(`fdatasync` 完了)。"windowed" = OS ページキャッシュ内にあり、ウィンドウ内で*マシン*(プロセスだけでなく)が死んだ場合にのみ失われる。

| 書き込みパス | `always` | `everysec` | `no` |
|---|---|---|---|
| サーバーコマンド応答 | 応答がシャードを出る前に durable(バッチごとにグループコミット) | windowed ≤ 1 s | OS ペース |
| 組み込みファサード操作(`set`、`zadd`、…) | リターン時に durable | windowed ≤ 1 s | OS ペース |
| 組み込み `atomic` / `atomic_all_shards` ブロック | コミット時に durable(触れたシャードごとに 1 fsync) | windowed ≤ 1 s | OS ペース |
| 組み込み `Pipeline::commit` | リターン時に durable、fsync はシャードごとにバッチ | windowed ≤ 1 s | OS ペース |
| …上記いずれか + **`Store::fsync_aof()`** | 無操作 | **バリアで durable** | **バリアで durable** |

`Store::fsync_aof()` は書き込み単位の耐久性エスケープハッチです(Postgres のトランザクション単位 `synchronous_commit` の類)。デプロイはスループットのために `everysec` で走らせ、確認された瞬間にマシンクラッシュを生き延びなければならない少数の書き込みの後ろにバリアを置きます。コスト: ダーティなシャードごとに 1 回の `fdatasync`。

プロセスクラッシュ(SIGKILL)は `always` の下では確認済み書き込みを決して失わず、それ以外では最大 fsync ウィンドウぶんを失います。AOF 末尾は次のオープンでリプレイされ、破れた最終フレームは隔離されます(`panic-quarantine`)— サイレントに適用されることはありません。

## アトミック性憲章(組み込み serving-store、v2.1)

- **`Store::atomic(body)`** — 単一シャードのトランザクション: クロージャの間そのシャードの書き込みロックを取り、内部の読み出しはクロージャ自身の書き込みを見え、AOF 追記は遅延されて**コミット時に 1 fsync** でコミットされます(`always` 下)。触れるすべてのキーは同じシャードにハッシュされなければなりません — したがって書き込みパターンが任意のキーにまたがる場合、serving-store の推奨構成は **1 シャード**です: 完全なアトミック性を保ち、クロスシャード調整を払いません。1 シャード構成の上限はシングルコア書き込みスループットです。実測値は `bench/REPORT.md` にあります。
- **`Store::atomic_all_shards(body)`** — マルチシャードのトランザクション: **すべての**シャードの書き込みロックをシャードインデックス順に取得し(決定的な順序 = デッドロックなし)、リターン時にシャードごとの AOF バッチをコミットします。コスト: クロージャの間、他のすべての読者と書き手をブロックします — クロスシャード不変条件のために使い、デフォルトの書き込みパスにしないでください。
- **`Store::pipeline()`** — アトミック**ではない**: 各操作が自分のロックを取り、他の書き手が交錯します。fsync をバッチする(N 操作 → シャード数以下の fsync)だけで、それ以上ではありません。
- どちらのアトミック形式も、条件付き操作(`ZADD GT`、`SPOP`)の**効果**を無条件の動詞としてログするため、リプレイとレプリカ適用は構造的に決定的です。

## リカバリポイント(v2.3)

変更フィードを有効にすると(`[feed] enabled = true`、[cdc.md](../cdc.md) を参照)、すべてのスナップショットは採取時点のフィードカーソルを記録します — スナップショットデータ自体と同じ追記禁止ウィンドウ内で凍結されます。これがリカバリポイント契約を与えます:

> **スナップショット S + S に記録されたカーソルからのフィードフレーム = それ以降の任意のカーソルにおける正確な状態。**

`kevy_persist::read_snapshot_cursor(path)` はカーソルを読み戻します(v2.3 以前のスナップショットは `None` — フォーマット v4 以前はカーソルを持たず、引き続き完全にロード可能)。この契約の実行可能な形は `bench/restore-drill.sh` で、`diskgate` の 1 行として走ります: 書き込み → SAVE → さらに書き込み → kill → dump だけからリストア → キャプチャしたフィードフレームをリプレイ → キーごとのバイト一致検証。

スコープ注: フィードウィンドウはインメモリのバックログです。ウィンドウより古いフレームは消えています — ウィンドウの届く範囲より古いスナップショットは、ただのスナップショットリストア(S 時点の状態)であり、PITR のベースではありません。正確な時点のリカバリに依存するなら、ウィンドウが一巡する頻度以上でスナップショットを取ってください。

## レプリケーションワイヤ上のスナップショット(v3.15)

同じスナップショットフォーマットが、バックログウィンドウから外れたレプリカにプライマリがインラインで送るものです([replication.md](replication.md) を参照)。知っておくべきセマンティクスがひとつ: 送られたスナップショットはレプリカのローカル状態を**置き換え**ます。マージではありません — レプリカはロード前に自分のキー空間をフラッシュします。これは意図的です: 再合流した旧プライマリが分岐サフィックス(一度もレプリケートされなかった書き込み)を抱えているとき、再同期は upsert のみのロードの下に残渣として残すのではなく、分岐を本当に破棄しなければなりません。
