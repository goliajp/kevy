...
**巨大な単一コレクションと rewrite 窓。**進行中の rewrite やスナップショット view に参照が固定されているコレクション値への書き込みは、サービングスレッド上で値全体のコピーオンライトクローンを支払う。通常の値ではマイクロ秒で不可視だが、GB 級に育った単一の list/set/hash では秒単位になる(soak 実測：複数 GB の単一キーへの継続追記下で 7-9 秒)。この規模の無制限な単一キューを保つワークロードでは、`LTRIM` 系で刈り込むか、そのキーのシャードで rewrite 窓中のレイテンシスパイクを織り込むこと；要素粒度の修正はロードマップにある。
