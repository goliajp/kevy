...
- **The standard library follows the dialect.** Each dialect gets the functions that dialect actually has, so a script that runs here runs on stock Lua of the same version. Under the 5.1 default that means `table.setn` exists (raising "'setn' is obsolete", as PUC does) and `table.unpack` / `table.create` do not — the 5.1 spelling is the global `unpack`. Ask for a dialect that has them with `#!lua version=5.4`. Before luna-core 3.0.0 every dialect saw the union of all five, which let scripts use names their target version does not have.
