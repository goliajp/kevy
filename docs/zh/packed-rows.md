...
索引让它**更差**，不是更好。规模也不是：同一探针在两百万行、无索引下是 1,522 → 1,576，即 **+3.5%**，与五十万行的数字相同。分片数两边一致（都是盒上默认），AOF 设置也一致。

三个候选因此全部排除，这个差异是**未解释的**。剩下的是完整基准做了而这个探针没做的那些事：流水线批量装载，以及装载之后的查询阶段。两者都还没被单独隔离出来；在此之前，诚实的读法是：表示法在「先声明」时值 −23.4%，而回填这条路的结果取决于本页尚未识别出的某个因素。
