...
**本页的两组测量彼此不一致，而且这个不一致还没有被解决。** 上面那张 AOF 模式表同样是「先装载后声明」，它**省** 13.5%；而这张表里同一个顺序**花** 3.5%。两者的差别在于规模（两百万行对五十万行），以及基准那张表还声明了索引，而索引自己的回填正好在打包回填刚刚释放完内存的地方大量分配。如果分配器把那些释放掉的表拿去复用而不是攥着，就能解释这件事——但还没有任何测量证实。把 −23.4% 读作表示法自己的效果；两个「先装载」的数字则一起说明：回填这条路的结果，取决于当时进程还在做什么。
