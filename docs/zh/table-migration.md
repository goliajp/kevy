...
**这里同样没有工具，而且这是「有工具反而有害」的那一处。** kevy 不追踪谁*读*过一个键，所以任何「已经没人读它了」的探测都是推断出来的，不是观测到的。这条教训的故障模式本来就是*静静地读到空、并认为没问题*——一个说「已确认，可以安全删除」的探测，等于给那个故障披上一件确认的外衣。请手工排删除的顺序：先 reader，再 writer，最后是键。
