...
- **已实测（2026-08-05）。** 比例曲线不再只是模型。同一预算、同一键长，只变值大小：
  ...
  地板是每条目约 96 B 的**键空间条目**（键长 9 B 时；键长 48 B 时约 143 B）——不是 stub，stub 本身 24 B 内联、不占堆；这 96 B 是每个键无论冷热都要付的，分层能还回值、还不回键。它在测过的每个尺度上都是平的，于是上限可以预测：**上限 data:RAM ≈ 值大小 /（96 B + key heap）**，对上面三行分别预测 2.67× / 10.7× / 42.7×。这条公式的交互版在 <https://kevy.golia.jp/zh/capacity/>。
