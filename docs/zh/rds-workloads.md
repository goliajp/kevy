...
不是断言——[脚本在此](../../bench/pgcompare.sh)。200 万行 / 854 MB CSV，同一套 harness 打两边，PG 用原厂默认，kevy 5.4 三轮交错取中位数：

| | kevy（`everysec`） | PG 18 默认 |
|---|---|---|
| 单行更新 p99 | **41 µs** | 1,676 µs |
| 单行更新 p99（**持久化对等**，每次写 fsync） | 3,339 µs | **1,676 µs** |
| 二级索引点查 p99 | 160 µs | 154 µs |
| 列表分页 p99（过滤 + 范围 + 排序 + 限量） | 200 µs | **162 µs** |
| 每 MB CSV 的内存 | 5,821 KB | **883 KB** |
...
**那第二行到哪里为止。** 「不替你决定」说的是*查询期*：不会有谁替你选计划，因为你的查询自己点名了它的路径。这不是「将来一个也不加」的拒绝。`TABLE.DECLARE … AUTODECLARE n` 允许引擎把一个你反复被拒的形状变成一条声明过的路径——不主动要就是关的、以你写下的 `n` 封顶、只在你声明过的列上、只增不减，并在 `IDX.LIST` 里标着 `auto`（[tables.md](tables.md#autodeclare你没写的那些路径)）。一个你预测不了的优化器，和一次你自己要求的 schema 变更，是两回事；这里拒绝的只有前者。

**有一条曾经在这张表里、而它不属于这里：`OFFSET`。** 它曾被列为拒绝；而引擎在带子句的那个面上（[tables.md](tables.md)）一直提供它——所以这张表写的是意图，不是行为。真实情况是这样的，在范围内 30 000 行上返回 `LIMIT 20` 实测（30 次调用的 p50）：
