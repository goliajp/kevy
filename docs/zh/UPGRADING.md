...
两章，新的在前：**3.x → 4.0**（一次 API 定义型的 major：线协议和磁盘格式照旧，Rust 接口面改了这一次，此后冻结）和 **2.x → 3.x**（一次能力型的 major：一切照旧带过去）。每一章都明确写清：什么自动升级、什么需要改代码、以及怎么退回去。
...
| `kevy-wasm` / `@goliajp/kevy`（npm） | —— | 4.0.0 | 4.0 新增——见 [wasm.md](wasm.md) |
...
**快照与 AOF。** 4.0 的二进制能读每一种 3.x（以及 2.x）的快照格式，并原样重放 3.x 的 AOF。格式在 4.0 里没有变——所以退回 3.18 同样只是换个二进制（这一点和 3.x → 2.x 方向不同，那个方向有一处格式边缘；见 2.x → 3.x 那一章）。
...
`core` 档可以交叉编译到 musl 目标，并扛着一份强制预算（二进制 ≤ 700 KB，空 store RSS ≤ 2 MB）；另有五个基础 crate 能构建 `no_std`。见 [iot.md](iot.md)。在体积谱的另一端，同一个 embedded 内核现在能以 `@goliajp/kevy` 的身份跑在浏览器里——见 [wasm.md](wasm.md)。
