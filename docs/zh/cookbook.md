...
具体地说：从闭包里返回 `Err` 会回滚它做过的每一次写入——内存里和 AOF 里都回滚——所以一个被拒绝的事务不留下任何痕迹，你也不必把闭包安排成「校验必须先于每一次写入」。（这条保证自 4.0 起才是真的；更早的版本会把被拒绝闭包的写入留在内存里，却丢掉它们的 AOF 帧，于是重启后的状态与运行中的进程对不上。）
