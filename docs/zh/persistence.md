...
**事务按全有或全无重放，与大小无关。** 嵌入式的 `atomic()` / `atomic_all_shards()` 块在 AOF 上被一个 begin 标记和一个 commit 标记夹住。重放会保留 begin 之后的每一帧，只有到达配对的 commit 时才把这一批应用下去；一条停在事务中间的日志会把它丢弃。拒绝这个块（返回 `Err`）则一个字节也不追加。

让这件事与大小无关的正是这两个标记，而这个区别很要紧。单靠组提交只是把 *fsync* 推后：AOF 经由一个 256 KiB 的缓冲区写出，所以更长的事务在缓冲区填满时照样把完整且合法的帧交给内核，而 `kill -9` 把它们留在那里。标记还不存在时的实测：一个含 20,000 次变更的块在提交中途被杀，重放出来 6,393 次。**没有任何 `appendfsync` 设置能修好它**——部分状态来自提交循环的形状，不是来自它什么时候 sync。

`appendfsync` 管的是另一件事：已经写下的数据的掉电窗口。它与「事务是否全有全无」正交。

两者自 4.0 起都为真。更早的版本把组提交写进了文档却从未启用，于是事务里的每一次变更都是单独 sync、并且单独重放的。

v1 格式的日志没有记录信封，表达不了事务边界；在首次重写把它们升格为 v2 之前，它们保持旧行为。

**标记属于事务，不属于别的任何东西。** 一个流水线化的读批次不是事务——Redis 的流水线明确是非原子的——所以反应器的批次不带标记（它们只是在 `always` 下共享一次 fsync；4.x 线上曾有一小段时间，连单命令批次都要付那对约 65 B 的标记，被一道磁盘门抓到了）。在服务器一侧，`MULTI`/`EXEC` 把排好队的命令夹在该连接所属的 shard 上；扇出到别的 shard 的命令，各自落进那些 shard 自己的日志里。所以崩溃下的跨 shard `EXEC` 原子性是**逐 shard 的，不是全局的**——与 Redis 自己那条「`EXEC` 内的运行时错误不会撤销其它命令」的规则同一个精神。上面那条全有全无的保证，属于写入在构造上就是单 shard 的嵌入式 `atomic()` 系列。
...
有一个值得知道的后果，来自某位用户的门禁上报：因为持久化下来的截止时刻是**墙上时钟**时间，写入与之后的重放之间若发生一次系统时钟跳变（NTP 校正），剩余 TTL 就会朝对应方向精确地移动那一步——向后跳读回来的剩余*更长*，向前跳则*更短*。这是绝对墙上截止时刻固有的性质（Redis 也一样，而且它在内存里就是如此）；kevy 运行中的 TTL 走的是单调时钟，不受影响——只有重启这一道边界要经由墙上时间换算。对于短命的限流器 TTL，请把跨重启后的剩余量，视作准确到你这台主机的时钟纪律为止。
...
**巨型单集合与 rewrite 窗口。**list、hash、set、sorted-set 超过约 1.6 万元素后采用元素粒度（分段）写时复制：rewrite 或快照 view 钉住值期间的写入只克隆命中的 ~1.6 万元素段，代价约 1 毫秒且与集合总尺寸无关（盒上实测：2000 万元素下首写 0.4-2.1 毫秒；此前的整值克隆要付 0.35-9.5 秒并短暂让该集合常驻内存翻倍）。Stream 是仅剩的例外：view 钉住期间对 stream 的写仍克隆整值，GB 级无界单键 stream 会在其 shard 上出现 rewrite 窗口延迟尖峰——如有此类键请用 `XTRIM` 修剪。
...
真正的临时开销在内存。快照写出期间被改动的集合只克隆写触碰到的部分，在线 store 不打扰冻结视图、继续前进。普通字符串键与 list/hash/set/sorted-set（按 ~1.6 万元素段克隆）最坏也只多占几 MB；只有 view 钉住期间被写的 stream 仍克隆整值，其常驻内存可能短暂翻倍。
...
| 文件中部一个文件兑现不了的长度头 | `LengthOutranFile`——一处坐在记录该在的位置上、而文件短得无法满足的截断 | 它之前的前缀；开启 `replay_resync` 后连同其后的完好尾巴 | 它之后的区域——**或**开启 `replay_resync` 后仅被跳过的那一段 |
...
有两类记录带着 NUL 前缀的名字，任何客户端打得出的 RESP 动词都不可能与之相撞：

- **事务括号**（`\0KEVYTXNBEGIN` / `\0KEVYTXNCOMMIT`）——让一组追加按全有全无重放。
- **段的缝合**（`\0KEVYSEGMENTED <file>`）——记录一批行从热层被逐出到 `segs-<shard>/` 下的一个冷段文件里。重放会把那次逐出重做一遍：这些行的写入帧在日志里排在缝合之前，重放把它们带进来，缝合再把它们请出去。排在缝合*之后*的写入帧是一次复活，就那样留着（热层优先的读取会遮住段里的那份副本）。

一旦数据目录里出现了 `segs-<shard>/`，**光凭 AOF 就不再是完整的真相**。真相集合是：快照（`dump-*.rdb`）＋ AOF（`aof-*.aof`）＋ 段目录（`segs-*/`，每个装着它的段文件和让它们成立的 `segs.manifest` 账本）。备份必须把三者都拷上；段一旦封存就不可变，所以增量备份就是「拷贝新的段文件和 manifest」。

段集合的真相之源是 manifest，不是缝合帧（manifest 在那一帧被写进日志*之前*就已 fsync）。两个后果：一个缝合帧点名了 manifest 并不持有的段，意味着段目录是事后被损坏的，于是启动会**按名字拒绝**，而不是默默丢掉那些唯一持久副本已经够不着的行；反过来，一个因快照截断或 AOF 重写而丢失的缝合帧是无害的——到那时热层已经不再持有那些行，或者（如果一次重写的视图在逐出中途 freeze 了）那些行在两层里都活着，而热层优先的读取会遮住段里的副本。
