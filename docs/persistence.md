...
**AOF writes and the reactor thread (5.0, io_uring default).** With io_uring available and a non-`always` policy, appends and the everysec fsync ride the shard's own ring as queued operations — the reactor never blocks in `write(2)` on the hot path, which is what used to park it for seconds under GB/s ingest (the 5.0 tail-latency work; measured end-to-end in `bench/`'s finding documents). Durability is unchanged: the `everysec` crash window is still ≤ 1 s, verified by the crash gate in both modes. `KEVY_AOF_OFFLOAD=0` restores the classic synchronous path; `always` keeps it by definition; the epoll reactor (older kernels) keeps it too.
