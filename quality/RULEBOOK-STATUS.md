...
| `naming/unit-in-the-name` | 全 | **COUNTED** | 340 — quantities whose name carries no unit (Duration/Instant excluded) |
...
| `fn/max-5-params` | 全 | **COUNTED** | 250 — functions taking more than 5 parameters |
| `fn/no-bare-bool-param` | 全 | **COUNTED** | 242 — bool parameters that are not the only parameter |
| `fn/result-not-panic` | 石钢边 | **LOCKED** | panic-free |
| `fn/no-out-param` | 全 | **COUNTED** | 546 — &mut collection parameters — SITES TO JUDGE, not violations: the rule allows a reused buffer on a hot path when the doc says why |
...
| `mod/mod-rs-assembles-only` | 全 | **COUNTED** | 35 — assembly points carrying logic, 371 functions between them |
...
| `type/no-catchall-match` | 石钢 | **COUNTED** | 635 — `_ =>` arms — SITES TO JUDGE: required on a foreign #[non_exhaustive] enum, a hole on one of ours |
| `type/no-stringly-typed` | 全 | **COUNTED** | 33 — branches taken on a string literal comparison |
