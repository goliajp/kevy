...
fn timeout_bounds_a_wait_alongside_a_pending_read() {
    ...
    let ts = KernelTimespec::from_millis(5);
    // SAFETY: `buf` is a live local and the length passed is what it holds; the fd is
    // open for the rest of this test, so both outlive the SQE until it is reaped.
...
fn reads_a_file() {
    ...
    let mut buf = [0u8; 64];
    // SAFETY: `buf` is a live local array, `file` stays open for the rest of the test,
    // and the length passed is `buf.len()` — the SQE is reaped before either drops.
    unsafe {
...
fn echo_round_trip_via_io_uring() {
    ...
    // read the request
    let mut rbuf = [0u8; 64];
    // SAFETY: `rbuf` is a live local and `conn_fd` is open until the end of this test;
    // the length passed is `rbuf.len()`.
    ...
    // write the echo back
    // SAFETY: same buffer and fd, still live; four bytes is inside `rbuf`.
...
fn multishot_recv_with_provided_buffers() {
    ...
    let mut pbr = match ring.register_buf_ring(4, 64, 7) {
        ...
        Err(e) => {
            ...
            // SAFETY: `conn_fd` was accepted in this test and never wrapped before, so taking
            // ownership here closes it exactly once on the skip path.
            let _ = unsafe { OwnedFd::from_raw_fd(conn_fd) };
            client.join().unwrap();
            return;
        }
