...
fn type_mapping_table() {
    ...
    let ty_of = |col: &str| {
        let i = argv.windows(2).position(|w| w[0] == "COLUMN" && w[1] == col).unwrap();
        argv[i + 2].clone()
    };
    ...
    assert!(
        c.notes.iter().any(|n| n.contains("m.d: serial") && n.contains("do NOT auto-increment"))
    );
...
fn schema_errors_are_named() {
    ...
    let e =
        compile("CREATE TABLE t (a bigint PRIMARY KEY); CREATE INDEX ON t (nope);").unwrap_err();
    assert!(e.message.contains("unknown column 'nope'"), "{e}");
    let e = compile(
        "CREATE TABLE t (a bigint PRIMARY KEY); CREATE VIEW v AS SELECT * FROM ghost WHERE a = 1;",
    )
    .unwrap_err();
    assert!(e.message.contains("FROM unknown table 'ghost'"), "{e}");
    let e =
        compile("CREATE TABLE t (a bigint PRIMARY KEY); CREATE TABLE t (a bigint PRIMARY KEY);")
            .unwrap_err();
    ...
    let e = compile(
        "CREATE TABLE t (a bigint PRIMARY KEY); CREATE INDEX ON t (a); CREATE INDEX ON t (a);",
    )
    .unwrap_err();
...
fn missing_access_path_names_the_fix() {
    ...
    assert!(
        e.message.contains("WHERE (dept EQ, age range) matches no declared access path"),
        "{e}"
    );
...
fn strict_bounds_adjust_integers_exactly() {
    ...
    let c =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE age > 30 AND age < 40;"))
            .unwrap();
    assert_eq!(
        leaf_of(&c),
...
fn open_ranges_fill_type_extremes() {
    ...
    let c =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE salary <= 9.5;")).unwrap();
...
fn literal_typing_is_checked() {
    ...
    let e =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE age = 'x';")).unwrap_err();
    assert!(e.message.contains("column 'age' is i64"), "{e}");
    let e =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE dept = 42;")).unwrap_err();
...
fn engine_caps_are_named() {
    ...
    let e = compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE dept = $1 LIMIT 0;"))
        .unwrap_err();
    assert!(e.message.contains("LIMIT 0"), "{e}");
    let e =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE dept = $1 LIMIT 20000;"))
            .unwrap_err();
...
fn select_and_order_validate_columns() {
    let base = format!("{DEPT}CREATE INDEX ON emp (dept);\n");
    let e = compile(&format!("{base}CREATE VIEW v AS SELECT ghost FROM emp WHERE dept = $1;"))
        .unwrap_err();
    ...
    let e =
        compile(&format!("{base}CREATE VIEW v AS SELECT * FROM emp WHERE ghost = 1;")).unwrap_err();
...
fn render_script_shape() {
    ...
    assert!(
        s.contains("VIEW.CREATE paid_orders QUERY orders.status EQ paid ORDER BY orders.status"),
        "{s}"
    );
