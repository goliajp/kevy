...
fn conn_and_introspection() {
    ...
    assert_eq_reply(&run(&mut s, &[b"PING", b"hi"]), b"$2\r\nhi\r\n", "PING/1");
    assert_starts(&run(&mut s, &[b"PING", b"a", b"b"]), b"-ERR", "PING/2");

    // ECHO
    assert_eq_reply(&run(&mut s, &[b"ECHO", b"hello"]), b"$5\r\nhello\r\n", "ECHO/1");
    ...
    assert_starts(&run(&mut s, &[b"CONFIG", b"SET", b"k", b"v"]), b"-ERR", "CONFIG SET");
    ...
    assert_starts(&run(&mut s, &[b"NOPE_NOT_A_CMD"]), b"-ERR unknown command", "unknown verb");
}
...
fn string_set_get_family() {
    ...
    assert_eq_reply(&run(&mut s, &[b"SET", b"k", b"v", b"EX", b"100"]), b"+OK\r\n", "SET EX");
    assert_eq_reply(&run(&mut s, &[b"SET", b"k", b"v", b"PX", b"123"]), b"+OK\r\n", "SET PX");
    assert_starts(&run(&mut s, &[b"SET", b"k", b"v", b"EX"]), b"-ERR", "SET EX missing arg");
    assert_starts(&run(&mut s, &[b"SET", b"k", b"v", b"EX", b"abc"]), b"-ERR", "SET EX bad int");
    assert_starts(&run(&mut s, &[b"SET", b"k", b"v", b"BOGUS"]), b"-ERR", "SET unknown opt");
    assert_starts(&run(&mut s, &[b"SET", b"k", b"v", b"NX", b"XX"]), b"-ERR", "SET NX+XX");
    // NX skip & XX skip paths.
    assert_eq_reply(&run(&mut s, &[b"SET", b"k", b"v2", b"NX"]), b"$-1\r\n", "SET NX skip");
    let _ = run(&mut s, &[b"DEL", b"missing_xx"]);
    assert_eq_reply(&run(&mut s, &[b"SET", b"missing_xx", b"v", b"XX"]), b"$-1\r\n", "SET XX skip");

    // SETNX (own verb)
    assert_eq_reply(&run(&mut s, &[b"SETNX", b"nx", b"v"]), b":1\r\n", "SETNX new");
    assert_eq_reply(&run(&mut s, &[b"SETNX", b"nx", b"w"]), b":0\r\n", "SETNX exist");
    assert_starts(&run(&mut s, &[b"SETNX", b"x"]), b"-ERR", "SETNX/1");

    // SETEX / PSETEX
    assert_eq_reply(&run(&mut s, &[b"SETEX", b"se", b"100", b"v"]), b"+OK\r\n", "SETEX");
    assert_eq_reply(&run(&mut s, &[b"PSETEX", b"pse", b"1000", b"v"]), b"+OK\r\n", "PSETEX");
    assert_starts(&run(&mut s, &[b"SETEX", b"k", b"v"]), b"-ERR", "SETEX/2");
    assert_starts(&run(&mut s, &[b"SETEX", b"k", b"abc", b"v"]), b"-ERR", "SETEX bad ttl");
    assert_starts(&run(&mut s, &[b"SETEX", b"k", b"0", b"v"]), b"-ERR", "SETEX zero ttl");

    // APPEND
    let _ = run(&mut s, &[b"DEL", b"ap"]);
    assert_eq_reply(&run(&mut s, &[b"APPEND", b"ap", b"foo"]), b":3\r\n", "APPEND new");
    assert_eq_reply(&run(&mut s, &[b"APPEND", b"ap", b"bar"]), b":6\r\n", "APPEND ext");
    ...
    assert_eq_reply(&run(&mut s, &[b"GETSET", b"gs", b"b"]), b"$1\r\na\r\n", "GETSET");
    assert_eq_reply(&run(&mut s, &[b"GETSET", b"never", b"v"]), b"$-1\r\n", "GETSET miss");
    ...
    assert!(run(&mut s, &[b"INCRBYFLOAT", b"f", b"1.5"]).starts_with(b"$"), "INCRBYFLOAT initial");
    assert_starts(&run(&mut s, &[b"INCRBYFLOAT", b"f", b"NaN"]), b"-ERR", "INCRBYFLOAT NaN");
    assert_starts(&run(&mut s, &[b"INCRBYFLOAT", b"f"]), b"-ERR", "INCRBYFLOAT/1");
}
...
fn string_counters_and_wrongtype() {
    ...
    assert_starts(&run(&mut s, &[b"INCRBY", b"n", b"abc"]), b"-ERR", "INCRBY bad int");
...
fn hash_full_surface() {
    ...
    assert_eq_reply(&run(&mut s, &[b"HSET", b"h", b"f1", b"v1"]), b":1\r\n", "HSET new");
    assert_eq_reply(
        &run(&mut s, &[b"HSET", b"h", b"f1", b"v1b", b"f2", b"v2"]),
        b":1\r\n",
        "HSET upd+new",
    );
    assert_starts(&run(&mut s, &[b"HSET", b"h", b"f1"]), b"-ERR", "HSET odd pairs");

    // HSETNX twice + arity-err.
    assert_eq_reply(&run(&mut s, &[b"HSETNX", b"h", b"f3", b"v3"]), b":1\r\n", "HSETNX new");
    assert_eq_reply(&run(&mut s, &[b"HSETNX", b"h", b"f3", b"v3b"]), b":0\r\n", "HSETNX exist");
    assert_starts(&run(&mut s, &[b"HSETNX", b"h", b"f3"]), b"-ERR", "HSETNX/2");

    // HGET / HEXISTS / HLEN happy + arity err.
    assert_eq_reply(&run(&mut s, &[b"HGET", b"h", b"f1"]), b"$3\r\nv1b\r\n", "HGET hit");
    assert_eq_reply(&run(&mut s, &[b"HGET", b"h", b"absent"]), b"$-1\r\n", "HGET miss field");
    assert_starts(&run(&mut s, &[b"HGET", b"h"]), b"-ERR", "HGET/1");
    assert_eq_reply(&run(&mut s, &[b"HEXISTS", b"h", b"f1"]), b":1\r\n", "HEXISTS");
    assert_eq_reply(&run(&mut s, &[b"HEXISTS", b"h", b"x"]), b":0\r\n", "HEXISTS miss");
    ...
    assert!(run(&mut s, &[b"HGETALL", b"h"]).starts_with(b"*"), "HGETALL");
    ...
    assert_eq_reply(&run(&mut s, &[b"HINCRBY", b"h", b"counter", b"5"]), b":5\r\n", "HINCRBY new");
    assert_eq_reply(&run(&mut s, &[b"HINCRBY", b"h", b"counter", b"-2"]), b":3\r\n", "HINCRBY dec");
    assert_starts(&run(&mut s, &[b"HINCRBY", b"h", b"counter", b"oops"]), b"-ERR", "HINCRBY bad");
    assert_starts(&run(&mut s, &[b"HINCRBY", b"h", b"counter"]), b"-ERR", "HINCRBY/2");

    // HDEL happy + arity.
    assert!(run(&mut s, &[b"HDEL", b"h", b"f1", b"absent"]).starts_with(b":"), "HDEL",);
    assert_starts(&run(&mut s, &[b"HDEL", b"h"]), b"-ERR", "HDEL/1");

    // Wrong-type: hash command on a string key.
    let _ = run(&mut s, &[b"SET", b"str", b"x"]);
    assert_starts(&run(&mut s, &[b"HSET", b"str", b"f", b"v"]), b"-WRONGTYPE", "HSET on string");
    assert_starts(&run(&mut s, &[b"HGET", b"str", b"f"]), b"-WRONGTYPE", "HGET on string");
}
...
fn list_full_surface() {
    ...
    assert_eq_reply(&run(&mut s, &[b"RPUSH", b"l", b"a", b"b", b"c"]), b":3\r\n", "RPUSH");
    assert_eq_reply(&run(&mut s, &[b"LPUSH", b"l", b"z"]), b":4\r\n", "LPUSH");
    ...
    assert_eq_reply(&run(&mut s, &[b"LINDEX", b"l", b"0"]), b"$1\r\nz\r\n", "LINDEX hit");
    assert_eq_reply(&run(&mut s, &[b"LINDEX", b"l", b"99"]), b"$-1\r\n", "LINDEX miss");
    assert_starts(&run(&mut s, &[b"LINDEX", b"l", b"abc"]), b"-ERR", "LINDEX bad int");
    assert_starts(&run(&mut s, &[b"LINDEX", b"l"]), b"-ERR", "LINDEX/1");
    assert!(run(&mut s, &[b"LRANGE", b"l", b"0", b"-1"]).starts_with(b"*"), "LRANGE",);
    assert_starts(&run(&mut s, &[b"LRANGE", b"l", b"a", b"b"]), b"-ERR", "LRANGE bad int");
    assert_starts(&run(&mut s, &[b"LRANGE", b"l"]), b"-ERR", "LRANGE/1");

    // LSET happy + miss + bad-int + arity.
    assert_eq_reply(&run(&mut s, &[b"LSET", b"l", b"0", b"Z"]), b"+OK\r\n", "LSET");
    assert_starts(&run(&mut s, &[b"LSET", b"l", b"abc", b"v"]), b"-ERR", "LSET bad int");
    assert_starts(&run(&mut s, &[b"LSET", b"l"]), b"-ERR", "LSET/1");
    assert_starts(&run(&mut s, &[b"LSET", b"l", b"99", b"v"]), b"-ERR", "LSET oor");

    // LREM / LTRIM happy + bad-int + arity.
    assert!(run(&mut s, &[b"LREM", b"l", b"0", b"a"]).starts_with(b":"), "LREM",);
    assert_starts(&run(&mut s, &[b"LREM", b"l", b"abc", b"x"]), b"-ERR", "LREM bad int");
    assert_starts(&run(&mut s, &[b"LREM", b"l"]), b"-ERR", "LREM/1");

    assert_eq_reply(&run(&mut s, &[b"LTRIM", b"l", b"0", b"-1"]), b"+OK\r\n", "LTRIM");
    assert_starts(&run(&mut s, &[b"LTRIM", b"l", b"a", b"b"]), b"-ERR", "LTRIM bad int");
    ...
    assert!(run(&mut s, &[b"LPOP", b"l", b"2"]).starts_with(b"*"), "LPOP count",);
    // Count form on empty list returns nil array.
    assert_eq_reply(&run(&mut s, &[b"LPOP", b"absent", b"5"]), b"*-1\r\n", "LPOP count miss");
    assert_starts(&run(&mut s, &[b"LPOP", b"l", b"abc"]), b"-ERR", "LPOP bad int");
    assert_starts(&run(&mut s, &[b"LPOP", b"l", b"-1"]), b"-ERR", "LPOP neg");
    assert_starts(&run(&mut s, &[b"LPOP"]), b"-ERR", "LPOP/0");

    // Wrong-type: list command on string key.
    let _ = run(&mut s, &[b"SET", b"str", b"x"]);
    assert_starts(&run(&mut s, &[b"LPUSH", b"str", b"v"]), b"-WRONGTYPE", "LPUSH on string");
}
...
fn set_full_surface() {
    ...
    assert!(run(&mut s, &[b"SADD", b"S", b"a", b"b", b"c"]).starts_with(b":"), "SADD",);
    ...
    assert_eq_reply(&run(&mut s, &[b"SISMEMBER", b"S", b"a"]), b":1\r\n", "SISMEMBER hit");
    assert_eq_reply(&run(&mut s, &[b"SISMEMBER", b"S", b"z"]), b":0\r\n", "SISMEMBER miss");
    ...
    assert!(run(&mut s, &[b"SPOP", b"S", b"2"]).starts_with(b"*"), "SPOP count",);
    assert!(
        run(&mut s, &[b"SRANDMEMBER", b"S"]).starts_with(b"$")
            || run(&mut s, &[b"SRANDMEMBER", b"S"]).starts_with(b"$-1"),
        "SRANDMEMBER single",
    );
    assert!(run(&mut s, &[b"SRANDMEMBER", b"S", b"2"]).starts_with(b"*"), "SRANDMEMBER count",);
    assert_starts(&run(&mut s, &[b"SPOP", b"S", b"-1"]), b"-ERR", "SPOP neg");
    assert_starts(&run(&mut s, &[b"SPOP", b"S", b"abc"]), b"-ERR", "SPOP bad int");
    ...
    let _ = run(&mut s, &[b"SET", b"str", b"x"]);
    assert_starts(&run(&mut s, &[b"SADD", b"str", b"v"]), b"-WRONGTYPE", "SADD on string");
}
...
fn zset_full_surface() {
    ...
    assert_starts(&run(&mut s, &[b"ZADD", b"z", b"x", b"a"]), b"-ERR", "ZADD bad float");
    assert_starts(&run(&mut s, &[b"ZADD", b"z", b"1"]), b"-ERR", "ZADD/2 odd");
    ...
    assert_eq_reply(&run(&mut s, &[b"ZSCORE", b"z", b"absent"]), b"$-1\r\n", "ZSCORE miss");
    ...
    assert_eq_reply(&run(&mut s, &[b"ZRANK", b"z", b"absent"]), b"$-1\r\n", "ZRANK miss");
    assert_starts(&run(&mut s, &[b"ZRANK", b"z"]), b"-ERR", "ZRANK/1");

    assert!(run(&mut s, &[b"ZINCRBY", b"z", b"1.5", b"a"]).starts_with(b"$"), "ZINCRBY",);
    assert_starts(&run(&mut s, &[b"ZINCRBY", b"z", b"oops", b"a"]), b"-ERR", "ZINCRBY bad float");
    assert_starts(&run(&mut s, &[b"ZINCRBY", b"z", b"1"]), b"-ERR", "ZINCRBY/2");

    // ZRANGE: rank + WITHSCORES + bad arg.
    assert!(run(&mut s, &[b"ZRANGE", b"z", b"0", b"-1"]).starts_with(b"*"), "ZRANGE",);
    assert!(
    ...
    assert_starts(&run(&mut s, &[b"ZRANGE", b"z", b"a", b"b"]), b"-ERR", "ZRANGE bad int");
    ...
    assert!(
        run(&mut s, &[b"ZRANGEBYSCORE", b"z", b"(1", b"+inf", b"WITHSCORES"],).starts_with(b"*"),
    ...
    assert!(run(&mut s, &[b"ZCOUNT", b"z", b"0", b"10"]).starts_with(b":"), "ZCOUNT",);
    assert_starts(&run(&mut s, &[b"ZCOUNT", b"z", b"x", b"y"]), b"-ERR", "ZCOUNT bad");
    assert_starts(&run(&mut s, &[b"ZCOUNT", b"z", b"0"]), b"-ERR", "ZCOUNT/2");

    // ZREM
    assert!(run(&mut s, &[b"ZREM", b"z", b"a", b"b"]).starts_with(b":"), "ZREM",);
    assert_starts(&run(&mut s, &[b"ZREM", b"z"]), b"-ERR", "ZREM/1");

    // Wrong-type: zset command on a string key.
    let _ = run(&mut s, &[b"SET", b"str", b"x"]);
    assert_starts(&run(&mut s, &[b"ZADD", b"str", b"1", b"a"]), b"-WRONGTYPE", "ZADD on string");
}
...
fn generic_keyspace() {
    ...
    assert_eq_reply(&run(&mut s, &[b"EXISTS", b"b", b"absent"]), b":1\r\n", "EXISTS multi");
    assert_starts(&run(&mut s, &[b"DEL"]), b"-ERR", "DEL/0");
    assert_starts(&run(&mut s, &[b"EXISTS"]), b"-ERR", "EXISTS/0");

    // EXPIRE / PEXPIRE / PERSIST / TTL / PTTL.
    assert_eq_reply(&run(&mut s, &[b"EXPIRE", b"b", b"100"]), b":1\r\n", "EXPIRE");
    assert_eq_reply(&run(&mut s, &[b"PEXPIRE", b"b", b"50000"]), b":1\r\n", "PEXPIRE");
    assert!(run(&mut s, &[b"EXPIRE", b"absent", b"100"]).starts_with(b":0"), "EXPIRE miss",);
    assert_starts(&run(&mut s, &[b"EXPIRE", b"b", b"abc"]), b"-ERR", "EXPIRE bad int");
    ...
    assert_eq_reply(&run(&mut s, &[b"TYPE", b"absent"]), b"+none\r\n", "TYPE miss");
