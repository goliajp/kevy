...
fn rpoplpush_moves_tail_of_src_to_head_of_dst() {
    ...
    let r = dispatch(&mut store, &argv(&[b"RPOPLPUSH", b"src", b"dst"]));
    assert_eq!(r, b"$1\r\nc\r\n");
    // src is now [a, b], dst is now [c, x]
    let g = dispatch(&mut store, &argv(&[b"LRANGE", b"src", b"0", b"-1"]));
    assert_eq!(g, b"*2\r\n$1\r\na\r\n$1\r\nb\r\n");
...
fn rpoplpush_empty_src_returns_nil() {
    let mut store = Store::new();
    let r = dispatch(&mut store, &argv(&[b"RPOPLPUSH", b"absent", b"dst"]));
    assert_eq!(r, b"$-1\r\n");
}
...
fn rpoplpush_same_key_rotates() {
    ...
    let r = dispatch(&mut store, &argv(&[b"RPOPLPUSH", b"l", b"l"]));
    assert_eq!(r, b"$1\r\nc\r\n");
    // After rotation: [c, a, b]
    let g = dispatch(&mut store, &argv(&[b"LRANGE", b"l", b"0", b"-1"]));
    assert_eq!(g, b"*3\r\n$1\r\nc\r\n$1\r\na\r\n$1\r\nb\r\n");
}
...
fn rpoplpush_dst_wrong_type_errors_without_consuming() {
    ...
    dispatch(&mut store, &argv(&[b"SET", b"dst", b"str"]));
    let r = dispatch(&mut store, &argv(&[b"RPOPLPUSH", b"src", b"dst"]));
    assert!(r.starts_with(b"-WRONGTYPE "));
    // src must still hold "a" because the pop got rolled back.
    let len = dispatch(&mut store, &argv(&[b"LLEN", b"src"]));
    assert_eq!(len, b":1\r\n");
}
...
fn lpos_default_returns_first_match_index() {
    ...
    let r = dispatch(&mut store, &argv(&[b"LPOS", b"l", b"b"]));
    assert_eq!(r, b":1\r\n");
}
...
fn lpos_absent_element_returns_nil_without_count() {
    ...
    let r = dispatch(&mut store, &argv(&[b"LPOS", b"l", b"zzz"]));
    assert_eq!(r, b"$-1\r\n");
}
...
fn lpos_count_returns_array_of_indices() {
    ...
    // COUNT 0 = all matches
    let r = dispatch(&mut store, &argv(&[b"LPOS", b"l", b"b", b"COUNT", b"0"]));
    assert_eq!(r, b"*3\r\n:1\r\n:3\r\n:5\r\n");
}
...
fn lpos_rank_negative_scans_from_tail() {
    ...
    let r = dispatch(&mut store, &argv(&[b"LPOS", b"l", b"b", b"RANK", b"-1"]));
    assert_eq!(r, b":3\r\n");
}
