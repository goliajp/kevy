...
/// The index-only answer and the row-read answer must be the same bytes.
///
/// When an index covers every requested field, `FIELDS` is served from the
/// segment's own copies and no row is touched. That is only worth anything
/// if the reply is indistinguishable from the one the row read produced —
/// including for a row whose covered field is missing, which must still
/// encode as nil rather than being dropped.
///
/// The two paths are forced apart by a field TTL: its presence anywhere
/// disables the index-only path (a sampling reaper cannot promise the copy
/// is current), so the same query answers from rows instead. Same query,
/// same data, both paths, byte-identical.
#[test]
fn the_index_only_page_is_byte_identical_to_the_row_read_page() {
    let srv = Server::start();
    let mut c = srv.connect();

    for i in 0..30 {
        cmd(&mut c, &[b"HSET", format!("co:{i}").as_bytes(), b"n",
                      format!("{i}").as_bytes(), b"tag", format!("T{i}").as_bytes()]);
    }
    // One row without the covered field: it must encode as nil on both paths.
    cmd(&mut c, &[b"HSET", b"co:99", b"n", b"99"]);

    let r = cmd(&mut c, &[b"IDX.CREATE", b"cix", b"ON", b"PREFIX", b"co:", b"FIELD", b"n",
                          b"TYPE", b"i64", b"KIND", b"range", b"VALUES", b"tag"]);
    assert_eq!(r, b"+OK\r\n");

    let q: &[&[u8]] = &[b"IDX.QUERY", b"cix", b"RANGE", b"0", b"99", b"LIMIT", b"40",
                        b"FIELDS", b"tag"];
    let index_only = query_ready(&mut c, q);
    assert!(String::from_utf8_lossy(&index_only).contains("T7"), "covered value present");

    // A field TTL anywhere sends the same query down the row-read path.
    cmd(&mut c, &[b"HSET", b"gate:1", b"f", b"v"]);
    cmd(&mut c, &[b"HPEXPIRE", b"gate:1", b"100000", b"FIELDS", b"1", b"f"]);
    let row_read = cmd(&mut c, q);

    assert_eq!(
        index_only, row_read,
        "index-only and row-read replies differ:\n  index-only: {}\n  row-read:   {}",
        String::from_utf8_lossy(&index_only),
        String::from_utf8_lossy(&row_read)
    );
}
