...
pub(super) const ROWS: &[VerbMeta] = &[
    ...
    v("SPOP",        "set", -2, W, "Remove and return one or more random members of a set.", "1.0.0", "SPOP key [count]",
      ...
      "differs: members are NOT random — SPOP returns the first members in hash-bucket order and the seed is a fixed constant, so repeated SPOP on the same set is fully deterministic"),
    v("SRANDMEMBER", "set", -2, R, "Return one or more random members of a set without removing them.", "1.0.0", "SRANDMEMBER key [count]",
      "O(count) expected",
      "differs: NOT random — it returns the same first-bucket member every time for a given set; a negative count is rejected instead of returning |count| members with repetitions"),
