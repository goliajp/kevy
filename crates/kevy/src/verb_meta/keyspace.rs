...
pub(super) const ROWS: &[VerbMeta] = &[
    ...
    v("TYPE",        "generic", 2,  R, "Return the type of the value stored at key.", "1.0.0", "TYPE key",
      "O(1)",
      "full"),
    v("COPY",        "generic", -3, W, "Copy a key's value, and its remaining TTL, to another key.", "1.0.0", "COPY source destination [REPLACE]",
      "O(1) same-shard; O(V) across shards, where V is the value copied over the ring",
      "differs: the DB argument is not accepted — kevy is single-database. A cross-shard copy is not atomic with anything else, but a failure between its two steps leaves both keys untouched"),
    v("TOUCH",       "generic", -2, R, "Count how many of the given keys exist.", "1.0.0", "TOUCH key [key ...]",
      "O(N) keys — one existence probe per key on its owning shard",
      "differs: kevy has no idle-time clock to reset, so TOUCH is EXISTS with the eviction bookkeeping the probe already does"),
    ...
    v("APPEND",      "string", 3,  W, "Append bytes to a string value; returns the new length.", "1.0.0", "APPEND key value",
      ...
      "full"),
    v("BITOP",       "string", -4, W, "Combine strings bitwise into a destination key.", "1.0.0", "BITOP AND|OR|XOR|NOT destkey key [key ...]",
      "O(N) over the longest source; on a multi-shard server each source is read on its own shard and the result written on the destination's",
      "differs: the DIFF / DIFF1 / ANDOR / ONE operators of Redis 8.2 are a syntax error"),
    v("BITCOUNT",    "string", -2, R, "Count set bits in a string, optionally over a byte range.", "1.0.0", "BITCOUNT key [start end]",
      "O(N) over the counted bytes",
      "differs: the BYTE / BIT unit argument of Redis 7.0 is not accepted — ranges are byte offsets"),
    v("BITPOS",      "string", -3, R, "Return the position of the first bit set to 0 or 1.", "1.0.0", "BITPOS key bit [start [end]]",
      "O(N) over the scanned bytes",
      "differs: the BYTE / BIT unit argument of Redis 7.0 is not accepted — ranges are byte offsets"),
    v("GETBIT",      "string", 3,  R, "Return the bit at the given offset of a string.", "1.0.0", "GETBIT key offset",
      "O(1)",
      "full"),
    v("GETEX",       "string", -2, W, "Read a key and set its expiry in the same call.", "1.0.0", "GETEX key [EX seconds | PX milliseconds]",
      "O(1)",
      "differs: EXAT / PXAT / PERSIST are not accepted, and an expiry of zero or less is refused rather than applied"),
    v("GETRANGE",    "string", 4,  R, "Return a byte range of a string value.", "1.0.0", "GETRANGE key start end",
      "O(M) over the returned bytes",
      "full"),
    v("SETBIT",      "string", 4,  W, "Set the bit at the given offset, returning its old value.", "1.0.0", "SETBIT key offset value",
      "O(1) amortised; O(N) on the call that grows the value out to the offset",
      "full"),
    v("SETRANGE",    "string", 4,  W, "Overwrite a string from the given offset, returning the new length.", "1.0.0", "SETRANGE key offset value",
      "O(M) over the written bytes; O(N) on the call that grows the value out to the offset",
      "full"),
