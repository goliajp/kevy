...
pub(super) const ROWS: &[VerbMeta] = &[
    ...
    // ---- table -----------------------------------------------------
    v("TABLE.DECLARE", "table", -9, WX, "Declare a relational table that compiles at declare time into named indexes (catalog mutation, sidecar-persisted).", "4.0.0", "TABLE.DECLARE name PREFIX prefix PK col COLUMN name i64|f64|str [COLUMN ...] [INDEX col range|unique [VALUES col ...]] [ORDERPATH name ON col [DESC] [THEN col [DESC]] ...]",
    ...
    v("TABLE.ENSURE",  "table", -9, WX, "TABLE.DECLARE's boot form: identical existing declaration answers +UNCHANGED, a different one is a named refusal saying which part differs — never a silent rebuild.", "4.1.0", "TABLE.ENSURE name PREFIX prefix PK col COLUMN name i64|f64|str [COLUMN ...] [INDEX ...] [ORDERPATH ...]",
    ...
    v("TABLE.REPLACE", "table", -9, WX, "Drop and redeclare in one verb, rebuilding every compiled index from the rows. Named for its cost: a full backfill, asked for explicitly. A bad spec refuses BEFORE the old table drops.", "4.1.0", "TABLE.REPLACE name PREFIX prefix PK col COLUMN name i64|f64|str [COLUMN ...] [INDEX ...] [ORDERPATH ...]",
