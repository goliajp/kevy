...
pub(super) const ROWS: &[VerbMeta] = &[
    ...
    v("IDX.DESCRIBE", "index", 2, RX, "Read an index declaration back: prefix, kind, type, fields, stored values, options, the table that compiled it, and the IDX.CREATE that recreates it.", "6.5.0", "IDX.DESCRIBE name",
      "O(declaration size), plus O(tables) compiles to name the owning table; at the origin — the catalog is process-global",
      "kevy-only"),
    ...
    v("TABLE.DESCRIBE", "table", 2, RX, "Read a table declaration back: columns with types, indexes, order paths, window, and the TABLE.DECLARE that recreates it (auto-declared paths are listed but left out of it).", "6.5.0", "TABLE.DESCRIBE name",
      "O(declaration size) at the origin — the catalog is process-global",
      "kevy-only: nearest is DESCRIBE / SHOW CREATE TABLE"),
    ...
    v("VIEW.DESCRIBE", "view", 2, RX, "Read a view declaration back: the composition tree, order, mode, hydration template, and the VIEW.CREATE that recreates it.", "6.5.0", "VIEW.DESCRIBE name",
      "O(declaration size) at the origin — the catalog is process-global",
      "kevy-only: nearest is SHOW CREATE VIEW"),
