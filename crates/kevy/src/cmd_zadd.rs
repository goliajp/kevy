...
fn parse_zadd_flags<A: ArgvView + ?Sized>(
    args: &A,
) -> Result<(ZaddFlags, bool, usize), &'static str> {
...
/// original `zadd_borrowed` fast path untouched.
pub(crate) fn cmd_zadd<A: ArgvView + ?Sized>(store: &mut Store, args: &A, out: &mut Vec<u8>) {
    ...
    if flags == ZaddFlags::default() {
        return emit_int_result(store.zadd_borrowed(&args[1], &pairs).map(|n| n as i64), out);
    }
    match store.zadd_flags_borrowed(&args[1], &pairs, flags) {
        Ok(rep) => {
