...
pub(crate) fn extension_op(store: &mut Store, argv: &[Vec<u8>]) -> Vec<u8> {
    ...
    // T6: the digest sweep is a bulk read — inside the peek scope a
    // cold row costs ONE record read (whole-value decode), never
    // promotes and never advances the 2nd-touch gate (RFC §4 row 15:
    // a full-prefix digest must not thrash the hot tier).
