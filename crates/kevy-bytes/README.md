...
dependencies, alloc-free for payloads ≤ 23 bytes.
...
- **Inline up to 23 bytes** — payload stored directly inside the value, with a sibling tag byte, inside the
...
let inline = SmallBytes::from_slice(b"redis");   // 5 ≤ 23 → no alloc
let heap   = SmallBytes::from_slice(&[0u8; 64]); // 64 > 23 → one alloc
