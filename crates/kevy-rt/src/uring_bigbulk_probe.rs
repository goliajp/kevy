//! RESP frame probes for the BigBulk
...
/// Defensive cap on the number of bulks the generic
...
pub(crate) enum BigArgGenericProbe {
    ...
    ///
    /// `body_start_in_tail`, `body_len`, and
    /// `bare_set_key_range` let the caller split a per-conn dest
    ...
    Promote {
        ...
        ///
        /// Consumed by the BigArgState machinery + the bare-SET fast
        /// path in `try_promote_bigbulk`.
        #[allow(dead_code)]
        ...
        #[allow(dead_code)]
        body_len: usize,
        /// When the promoted shape is exactly bare
        /// `*3\r\n$3\r\nSET\r\n$<klen>\r\n<key>\r\n$<bodylen>\r\n…`,
        /// `Some((start, end))` is the `tail` byte range of `<key>`.
        /// `None` for SETEX / PSETEX / APPEND / GETSET / MSET — those
        /// shapes still promote but the kernel-direct fast path is
        /// bare-SET only; broader variants are a possible follow-up.
        #[allow(dead_code)]
...
/// Generic last-bulk-big probe. See
...
/// position 3 of 5) returns `NotApplicable` — a possible follow-up.
...
pub(crate) fn probe_generic_bigbulk(buf: &[u8]) -> BigArgGenericProbe {
    ...
    // Capture the key bulk's content range for the bare-SET
