...
/// Maximum iovec entries packed into one `writev` SQE.
...
/// UDS: accept on the (shard-0-only) Unix-domain listener.
pub(crate) const OP_ACCEPT_UN: u64 = 7 << OP_SHIFT;
/// The cancel SQE issued to abort a multishot recv
...
/// The single-shot `prep_read` SQE that draws body
