...
### 决定 2 — BM25 统计保持 shard-local,加周期性全局快照 ⟵ **已被推翻(2026-07-23)**
...
**推翻(2026-07-23,t5.7 步骤 4b 收口)。** 上面的判据里藏着一个未被检验的
前提:**"全局统计"必然意味着写路径协调**。不必然 —— 落地的是**读时两趟**:
pass 1 只问各分片"这几个查询词各自的 df 是多少",origin 求和,pass 2 拿着总数
再打分。写路径一行没碰,也没有任何东西被缓存,于是**陈旧窗口根本不存在**。
两个被认为要二选一的东西同时拿到了。

**perf 也测了**(这条一直是"未实测"挂着的):同语料同查询,两趟的线上构建
vs 一个单趟(4b 之前行为)的实验构建,lx64 textgate 1M 文档 ——

| 构建 | MATCH p95(median-conn) | worst |
|---|---:|---:|
| 两趟(线上) | 28.19 ms | 32.60 ms |
| 单趟(实验) | 28.13 ms | 32.72 ms |

**多出的那一轮 fan-out 值 0.06 ms(0.2%),在噪声里**,worst-conn 那格两趟
反而更好。原因:pass 1 只搬**被查询的那几个词**的 `(token, df)`,而 pass 2 要
在百万文档上真扫——一个环回往返摊进 28 ms 等于零。

**附带义务随之反转**:文档**不得**写"N 秒陈旧窗口"(那会是假的),该写的是
"分数是全局的,单分片与八分片同排序同分数",并有 CI 里的 shard-invariance 测试
兜底。`docs/text-search.md` 已按此更正——它此前仍在宣称一个已不存在的局限。
