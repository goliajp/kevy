...
## 判决不许从管道的退出码里取(2026-08-13,downstream 送的)

`cmd | head` 之后读 `$?`,读到的是 `head` 的退出码 —— 门红被渲染成绿。
smix 这轮吃过一次,已把"验收命令一律直读 `$?`、不接管道"写进他们的发布
清单。kevy 侧按这条审了所有 `bench/*gate*.sh`:**没有找到判决从管道
退出码里取的真缺陷**(唯一的 `$?` 站点是 heredoc 后面,取的是 python 的
退出码,不是管道的)。

但顺手收紧了一处**隔空读 `$?`**:`availgate` 的 phase-4 seeder 原本是
`python3 ... <<'PYEOF'` + 三行注释 + `[ $? -eq 0 ] || fail`。隔着距离读
状态,一旦有人在中间插一条命令,它就会静默地开始汇报那条命令。已改成
把 `|| fail` 挂在命令自己身上——同文件四行之后的姐妹 seeder 本来就是
这个写法。
