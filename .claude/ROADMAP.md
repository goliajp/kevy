...
- [~] **C1 跨 shard block-serve 丢元素** — peek 修复对 **macOS-kqueue + uring 已确认,epoll 路径验证中(2026-07-23 设计 round,进行中)**。① escrow(2026-07-21)关"reply 回来时 origin 记录已没"窗口(`abandoned` → abort_serve 恢复)。② 第二窗口:poller 路径 EOF 只设 `conn.closing`、`close_conn`(设 abandoned)延到 flush reap,故 reply 可在断连被处理**前**到达 `origin_on_serve_resp`,`abandoned` 仍 false + socket 已死 → 旧码 deliver 进死 conn + 释放 escrow → 丢(uring 路径 EOF 时立即置 abandoned,无此窗口,所以 CI 只在 macOS 现)。修法 = 投递前 `Socket::peer_gone()`(非阻塞 `MSG_PEEK` 直接问内核)。**已验证**:macOS-kqueue 上 `KEVY_TEST_XSHARD_HOLD_CLOSE` seam 确定性"先红后绿"(去 peek `*0`/加 peek `*1`);uring 无此窗口;`peer_gone` 单测过(含 Linux)。**未闭合 = peek 方案的固有天花板**:Linux-epoll 回退实测 **18/20**(残留 ~10% `*0`)。由排除法确认残留是 **peek-alive**:`*0` 要求 abandoned=false + conn present + peer_gone=false = peek 点检返回"活着"但客户端已死。peek 是**投递决定时刻**问内核,而决定与实际 `ack_serve` 释放 escrow 非原子,点检可能变旧 —— kqueue 时序恰好每次赶上、epoll 偶尔漏(加 eprintln 探针即变过 = Heisenbug,坐实是时序非逻辑)。顺带修了独立 gap:reply 非空但 origin 记录已没 → escrow 搁浅(已按 key owning shard 恢复)。**完备修法 = write-result**(escrow 不在 deliver 时释放,绑定到 conn output flush 成功才释放、失败/拆除则恢复)—— 但改**两个 reactor 的写路径**(blast radius = 所有 reply 非仅 block serve),steel 层高风险,**需用户明确拍板才落**,非 autorun patch。**默认路径(Linux uring / macOS kqueue)已稳**,残留仅在显式 `KEVY_IO_URING=0` epoll 回退。我一度误标 `[x]` 已撤回。
