...
- **步骤 5 位置索引** → phrase / proximity / HIGHLIGHT(施工分子步,每子步 CI-可验;positions 内存公式 lx64/textgate 在步内验)
  ...
  - [~] **5d.2 phrase head+head** — **条目的诊断是错的**(2026-07-23)。它写着「需 positional galloping」,但 perf 剖面显示 **87% self-time 在 libc 分配器**:路径是**分配受限**不是扫描受限。先猜的双指针归并 -10.3%(已回滚);按剖面消掉`Positions::get` 的每候选每词一次 Vec 分配后 **-6.2%**(五对交替配对,区间不重叠,已 ship `9f85de2c`)。**galloping 本身仍未验** —— 分配消掉后它还有没有剩余收益没测过,故此条留半开而非勾掉
