...
- **步骤 5 位置索引** → phrase / proximity / HIGHLIGHT(施工分子步,每子步 CI-可验;positions 内存公式 lx64/textgate 在步内验)
  ...
  - **5e HIGHLIGHT spans**(用户 2026-07-21 授权大 epic 开工;分 5e.a stone / 5e.b wire)
    ...
    - [ ] **5e.b wire** — un-reserve HIGHLIGHT(args.rs:186 NOT_YET)+ 解析 `HIGHLIGHT [field…]`(与 FIELDS 两变长 clause 歧义,重构 MatchArgs::parse 逐 clause 扫描)+ thread highlight fields 过 pass-1→reduce→pass-2 + 每 hit 调 `highlight_spans` + 拓宽 reply row(`cmd_index_reduce/ranked.rs:64` merge_ranked / embedded `emit_ranked`)。RESP2 flat / RESP3 map(RFC §reply)
