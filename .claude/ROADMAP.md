...
- [ ] **死集合基线现在松了约 150 个 region**(基线 27,873,近几轮 Linux 观测
      27,721–27,753)。三个符号被钉死 + 三个 crate 的消融把它压下来了,而
      ratchet 只禁增长、不会自动跟降。收紧的正确做法是**在同一份代码上取三轮**
      重建包络(`setratchet.py envelope`)—— 单轮重录会比噪声还紧,下一轮就假红。
      自然时机是发布合入后 develop 上的头三次 covgate 运行。
