...
- [~] embedded bench RFC:竞品名单+轴 **RFC 已写**(`.claude/rfcs/2026-07-23-v4-embedded-bench.md`,四路竞品调研入账,marketing vs 第三方已标)。**一处更正 ROADMAP 候选**:C# 侧 LiteDB **降级为文档存储参考**(它无原生 KV 路径,只有 collection Insert/FindOne=品类错配),真正公平的 C# KV 对手 = **LMDB via Lightning.NET**(原生 mdb_put/mdb_get 同步)。选定:Go=bbolt(读型)+badger(LSM/append 近 kevy AOF);Node=better-sqlite3(同步,真 bar)+classic-level(异步,标记为跨模型参考);C=LMDB(读延迟王,最硬 bar);C#=LMDB via Lightning.NET。**公平性框架**:durability 三层(T-mem/T-async/T-fsync,只同层比)/ 同步异步不同表 / 冷单操作 vs 摊销双轴 / 值尺寸扫 16B-64KB + 冷启动。**四语言 harness 全建成 + 本机相对位次全测**(2026-07-24,bench/embeddedgate/{node,go,c,csharp},bench/EMBEDDED-LEDGER.md 四轨 + 跨轨综述):对五引擎(SQLite/bbolt/badger/LMDB×2)。**普适规律**:kevy 胜单操作读(kevy_get_shared 零拷贝 lane 平 ~12ns、**反超读延迟王 LMDB 2-9×**)+ 单操作小写(无每 op txn,12-62×);**输 bulk/batch 写**(每引擎都赢,bbolt 单事务 147×/LMDB 37×/SQLite 3.8× @64KB —— kevy 无 batch-write 路径)+ 大单写双拷贝 + 拷贝型绑定的读(Go/C# byte[] 拷出,C 轨道证零拷贝 lane 才是引擎真优势)。**攻击 #1 batch-write 已 BUILT+MEASURED**(2026-07-24):`kevy_set_many` FFI 原语(batch.rs,一次 crossing N sets,durability 不变)+ 接入 kevy-go `SetMany`(字节界 ~1MiB C arena,oversized 回退)+ Kevy.Embedded C# `SetMany`。**实测推翻"batch 处处闭合缺口"**:bulk 缺口是**引擎级**(C 无 crossing + C# 廉价 P/Invoke 都停 ~200ns vs LMDB ~80ns,set_many 均不动),根因 = kevy **每 op 格式化 AOF RESP frame**(它的 durability 模型:每写即入可恢复日志)vs LMDB 延迟到 commit 的 dirty-page 写 —— frame 格式化本质 per-op、batch 无法减。set_many **只帮昂贵-crossing 绑定**:Go cgo(50-100ns/op)从 **147×→2.70×**;C# 无救(廉价 crossing,本就引擎受限,arena 反微增)。**闭合引擎缺口 = 改 durability 模型(per-op 日志→事务/commit)= persistence-core RFC**(高 blast,设计轮先行,非 autorun)。测试:FFI 往返/misuse + kevy-go/C# SetMany 往返(含 2MiB oversized 回退)全绿。攻击 #2 零拷贝绑定读 lane 仍开(niche + lifetime-safety,defer)。LiteDB 品类错配(C# 轨 LMDB 验证)。**lx64 定值 pass 待**。**engine bulk 天花板已 RFC 定案**(`.claude/rfcs/2026-07-24-embedded-bulk-write-ceiling.md`):decompose 到根因 = kevy 每 op 格式化可恢复 AOF frame(它的 durability 模型),frame 格式化+hashmap insert 皆 per-op、EverySec 已批 fsync,**无改 durability 模型则无引擎级 win**;**决策 = 保留 per-op durable log,不加 deferred-commit batch 模式**(为 bulk 吞吐牺牲"每写即恢复"保证 = 对薪资级消费者的错误取舍)。2.5-2.7× bulk 差是 per-op durability 对"commit 前无 durability"引擎的诚实代价。Attack #1 **整段 COMPLETE**(考量后的 no,非待办)。**发现一既有失败(非我引入,已铁证)**:dispatch_oracle 本地失败(embedded RESP IDX.CREATE parser 缺 server 的 FTS 语法 FIELDS/VALUES),但 CI 在 f5e0293c 双平台实测绿 —— 环境级矛盾未 root(未接入 CI 精确构建前不盲改大 FTS parser)。**Attack #2 零拷贝绑定读 lane 已 BUILT for Go**(2026-07-24):kevy-go `GetView(key, fn)` 回调作用域零拷贝(view 仅 fn 内有效、返回即释放,镜像 bbolt txn-slice 契约);**闭合大值 GET 绑定输** —— 64KB 从 GetScalar 拷贝的输 62× → GetView 反超 1.04×,vs badger ValueCopy 胜 52×;GetView 全尺寸平 ~100ns(~12ns 引擎+固定 cgo crossing),仅小值 16B 仍略输 bbolt 进程内 mmap 指针(1.89×,crossing 地板)。测试 `TestGetViewZeroCopy`。**C# 同款已接**:`KevyDb.GetView`(scoped-delegate ReadOnlySpan,更安全),平 ~28-48ns(P/Invoke 比 cgo 更廉价),vs LMDB copy-out 胜 **1.8×→78× @64KB**;`getview-verify` 过。**Attack #2 现 BUILT for Go+C#**(两个拷贝型绑定;C 轨道无需 view lane,裸 C 已零拷贝)。**两个攻击面(batch-write / zero-copy read)现都整段落地 Go+C# + 引擎决策 RFC**。**dispatch_oracle 已修**(2026-07-24,`7dec633f`):根因 = embedded RESP `IDX.CREATE` parser 只支持旧单 `FIELD`、usage 串陈旧,server(FTS 弧后)支持 `FIELDS/WEIGHTS/WITH POSITIONS/VALUES/TYPES`;dispatch_oracle(embedded vs 真 spawn server 逐字节)抓到漂移。修 = 新建 `dispatch/idx_create.rs` 镜像 server parser + 路由到 Store 已有全能力 `idx_create_text`(多字段+权重+positions+values)/`idx_create_ann`/`idx_create_agg`/`idx_create`;错误串+CREATE_USAGE 逐字复制、DISTANCE 内联(判别值核对同 kevy_vector,免 vector 依赖);**server 与测试未碰**。`dispatch/idx.rs` 305→160。验证:dispatch_oracle ok + kevy-embedded 247 + kevy 全绿 + clippy(index/text/vector)+ locgate PASS。embedded 现经 `cmd()` 真接受 FTS IDX.CREATE 语法(非仅 typed API)。**拍板项**:Node napi 绑定补全(可选确认点)/ lx64 定值 / classic-level 保留
