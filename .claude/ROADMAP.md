...
- [ ] 正确刻度的下一攻:**单 shard forced-rewrite finish 分解**——插桩 PersistDone→swap 落定之间反应堆侧的每步(reopen / tee hand-off / generation 记账)对多 GB AOF 的耗时,具名后下刀;在此之前边界陈述成立(forced BGREWRITEAOF 在多 GB/shard 数据集上每 shard finish 可见 ~100-200ms tick;auto 路径稳态干净,rc-soak 98 次 ≤47ms)
