...
- [x] NuGet Kevy.Embedded 打包:runtimes/<rid>/native 三平台 cdylib;dotnet pack + 本地 feed 装机 smoke —— 打包+离线装机脚本(`pack-and-smoke.sh`:dotnet pack → 本地 feed → 不设 KEVY_FFI_LIB 让 .NET RID 图自行定位原生库 → set/get/reopen)本机 osx-arm64 跑通;**接进 CI 新 job `nuget-install-smoke`**(matrix: macOS-arm64 + ubuntu-x64,与 `npm-install-smoke` 同构,后者两腿 CI 已 success 证形态成立)。linux-arm64 无托管 runner 留自托管盒。真 `dotnet push` 属 t6
