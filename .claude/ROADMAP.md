...
- [ ] **N4' — 分解索引那 669 B/行**。两个索引 669 B ⇒ 每个约 334 B，而内存
      公式声明的每条目开销只有 48 B(`kevy-index/src/segment.rs`)——
      **大部分在公式没有命名的地方**。先测它是什么(段节点结构 / `IndexValue` /
      `ORDERPATH` 每行派生的复合键 是候选，不是结论)，再决定攻哪一项。
      **不许**照着任何成本表直接开工 —— 这一轮已经为此付了五次。
