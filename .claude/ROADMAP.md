...
- [ ] **T0 门先行**:RFC §2 验收标准逐条映射 gate 断言(对应表进 RFC §validation);`bench/tiergate.sh` + `bench/tablegate.sh` 骨架;透明套件 harness(dispatch_oracle 体裁,同 op 序 tiered vs untiered:语义命令逐字节、memory-reporting 命令 Shape 比对;untiered-vs-untiered 自检绿,tiered 模式 pending 至 T3 = 先红)。开工第一动 = feature 分支
...
- [ ] **T3 store 核心**:`Value::Cold(ColdRef{vlog_id,offset,len,weight,type_tag})` + 两段式 funnel(live_entry 族 accounting.rs:171,204 + typed helper 臂 hash.rs:376/list.rs:123/zset.rs:199/set.rs:87 + SET 裸探测 string_set.rs:96 的 Occupied-Cold 臂)+ `demote/promote_in_place`(不清 hfttl/零事件/不动 expires/保 lru_clock)+ evict_one 分叉(采样跳 Cold,spill 批 32 + tick 续,滞回)+ 二次访问 promote 门 + **最小 config 旋钮(env/TOML budget=<bytes>,足撑 gate)** + **compaction 触发接线(dead-bytes → shard tick)** + **KEVY_TEST_FORCE_DEMOTE 确定性 seam**;出口 = **B9 透明套件转绿(确定性降温,非水位时序)** + A1/A2 perfgate + B3 spill 预算/停顿 + B12 零事件计数器断言
...
- [ ] **T5 统一预算 + auto 探测 + INFO**:used_memory + Σ approx_bytes 统一水位;cgroup v2/meminfo/sysctl(kevy-sys 手绑 extern);floor 超预算 IDX.CREATE 具名拒;MEMORY USAGE 报 stub 实占;INFO gauges(cold_keys/cold_bytes/vlog_size/vlog_dead_bytes/demotions/promotions/index_bytes);两 config 面补全(T3 最小旋钮 → auto/百分比/INFO/embedded builder 平权;embedded 走新 `tier` feature,requires persist);出口 = B7 公式 memgate ±20%(冷键 ≈ ENTRY_OVERHEAD 96B + 键堆字节)+ B8 RSS 依从 + auto 双环境正确
...
- [ ] **T8 kevy-sql 编译器**:独立 crate(**手写递归下降解析器,0-dep,无 sqlparser**)+ `kevy-cli sql` 子命令;SQL 子集 v1(CREATE TABLE / CREATE [UNIQUE] INDEX / 单表 CREATE VIEW AS SELECT:索引列 WHERE + 声明列残余 FILTER + ORDER BY + LIMIT/OFFSET),其余具名报错;`docs/cookbook.md` 新章"porting a PG/MySQL schema"(真 schema 端到端);出口 = 编译器单测 + cookbook 章可执行 smoke
- [ ] **T9 容量终账**:**envelope 全部由 RFC 容量模型公式算出**(RAM_floor = keys×(96+键) + Σ索引×(68+VALUES 列)—— 50M-on-2GB 初稿正是没过公式被更正的教训):D1(**10M 行×1KiB ≈10GB 表 on 3GB + 2 索引 + VALUES,floor ≈2.8GB**,索引热行冷,C4/C5 保持)/ D2(全冷表 index-only 冷读=0)/ D4(混合负载隔离)于 lx64;B6 容量 10× gate **@4KiB 值**(5M×4KiB=20GB on 2GB;100× stretch 视盘容记录);B2 冷读 p99 线(标量 ≤100µs/300µs,hash 行 ≤200µs/500µs);memgate/diskgate 公式(B7/C8)定稿;docs 三语(tiering.md + tables.md + UPGRADING 注记);CHANGELOG 4.0.0 capacity 段;五轴收口 → merge
