...
- [x] **十三件仪器全绿,而八扇发布门停在旧版**(2026-08-30,v6.0.0 发布后)。
      ...
      树**,而树对自己说的是真话。没有任何门问过 registry。查出来:五个 npm
      绑定包停在 **5.1.0**(落后四个发布)、Maven 停在 **5.3.0**、官网停在
      **5.4.1**、`tauri-plugin-kevy-api` **从未发布**而 README 教人装它。
      再往回查:**v5.4.0 / v5.4.1 只到了 13 条渠道里的 4 条**。
      根因不是漏了步骤 —— 五个包**根本没有步骤**:tag workflow 的 npm job
      字面点名一个包,那五个是 5.1.0 时人手发的。
      八扇门补齐后 `check_channels_published.py` 问十类 registry,
      **门的清单从树推导**(手写清单正是 Maven 被漏两次的原因),
      **56 扇门全绿 / 5.4.1 红 12 扇 / 装不认识的目录退 2**;
      `channel-parity.yml` 每日跑,问最新 tag 不问树。
      `bench/FINDING-2026-08-30-every-gate-read-the-tree-none-asked-the-world.md`

- [x] **发出去的引擎写着构建者是谁**(同轮,被上一条的部署顺带挖出)。
      17 个预编译引擎每个都带 `/Users/<name>/…` —— Android `.so` 各 27 处、
      macOS xcframework 片 **422 处** —— 随三个移动端包发了出去,历次发布都带着。
      根因:本机装了 `rust-src`,rustc 把 std 的 panic 位置解析成本地源码路径,
      CI 没装所以从来没显形,两边产物差 4 KB 而没人看过那 4 KB 是什么。
      三个 `packaging/*.sh` 加 `--remap-path-prefix=$HOME=~`,17 个全部重建;
      对齐门第六层现在拒绝写着**本机** home 的构件 —— 判据不能认任何
      `/Users/`,`aarch64-apple-darwin` 的 compiler-builtins 里有 343 处
      Rust 官方构建机路径,红在正确的文件上门就没人看了。
      全产物审计闭合:剩余路径全是 GitHub runner,不指向任何人。

- [x] **自动化一件一直靠手做的事,先弄清它为什么一直靠手做**(同轮,自己引入自己接住)。
      新加的「发全部绑定包」步骤在 CI 干净 checkout 上会打出**声明了 `ios`/
      `android` 却两者皆无的包**:42,467 字节,而线上是 110,723,661,npm 照收 ——
      那些构件是 gitignored 的构建产物。移动端三个包人手发布**不是疏忽是必需**。
      发布步骤现在问 registry 现在服务多少字节,少于一半就拒绝;
      下限也从「find 到几个文件」移到「有几个可发布包」。

- [x] **一段配置说明它想做什么,不等于它在做那件事**(同轮)。`ci.yml` 的
      concurrency 注释写着「取消同分支的在途运行」,键却用 `github.ref` ——
      同提交的 push 是 `refs/heads/x`、PR 是 `refs/pull/N/merge`,两个组互不相干,
      于是每个 PR 在同一棵树上跑两遍 58 个 job 互抢 runner。改键为分支名后当场生效。

- [x] **`::srandmember` 登记为第 10 个抖动符号**(同轮)。deadgate 红在它
      `1 -> 2`,而那次运行整体**更好**(少 4 符号、少 37 死区)—— 逐元素最大值
      不许拿一个符号的改善抵另一个的退化,这是对的。同一提交同一棵树,
      第 1 次尝试红、第 2 次绿,零改动:变量是运行不是代码。用实验判,不用推理。
...
- [~] **死集合基线已重建为包络:27,873 → 27,732**(-141),用的正是这条写下的
      ...
      样子松一点。
      **这一步已在 2026-08-31 做掉了**:rustfmt 合入 develop 后,在**同一份代码
      (9ec62051)**上取了三次 covgate 读数 —— **26,834 / 26,967 / 26,854**,
      同码跨度 **133 个区域**。新包络 = 2,611 符号 / 26,969 区域。
      过程中我先犯了一个错,记在这里:处理 rustfmt 的 deadgate 红时,我用**单轮**
      `update --accept-growth` 记了基线 —— 那把基线收紧到噪声之下,随后 deadgate
      在三轮里**红了两次**,红的都是 `SegMap::rand_entry`(随机取样,每轮覆盖
      2〜4 个区域)。单轮基线是错的;包络说的是"不比三轮里最差的更差",
      这才是这里唯一有意义的基线。
...
- [x] **格式化已落地,并且有门在管了**(2026-08-30,属主拍板"做完")。
      此前:没有 `rustfmt.toml`、CI 里没有 fmt 步骤、`cargo fmt --all --check`
      报 4,634 处 / 772 个文件。
      **只设一个旋钮,而且是量出来的**:rustfmt 默认的
      `use_small_heuristics = "Default"` 会在 60% 宽度处就把调用/结构体/链
      拆开,而这棵树本来就是按 100 列手写的(中位 38、p95 78、p99 94)——
      默认设置给本来就放得下的代码加行,**足以把 76 个函数顶过 50 行、
      22 个文件顶过 500,共 98 处违规而一个 token 没变**。`"Max"` 只剩 19 处。
      `max_width` 保持 rustfmt 默认的 100:加宽到 110/120 能把余数降到 7/5,
      **没有采用** —— 采用 rustfmt 的意义就是不再有自家风格,而 100 是别的
      Rust 项目都在读的宽度。
      19 处的处理:**2 个文件先拆**(`kevy-ffi/src/lib.rs` 正好卡在 500、
      `replica_runner.rs` 489,都按它们本来就有的接缝拆),**15 个函数真拆**
      (多数顺手消掉一处重复:`start_single` 的两个同 body 拒绝块、两个 drain
      相同的 8 行开头、`parse_column_def` 里三种说法的同一个答案……),
      **2 个给 waiver**(`run_bulk_cli` 11 臂、`rebuild_seg_lists` 被 `#[cfg]`
      对折的 IndexKind 表)。有一次抽取**做了又撤回**:共享 Ping 分支需要 8 个
...
- [x] **crashgate T6 查清了 —— cell 是对的,引擎是错的**。仪器(保住被丢弃的
      ...
      **现在数够了:9 轮 CI、9 个不同提交、9 次 PASS、0 次 REDpending。**
      ...
      verdict 让它在出现的那一轮就可见。
      **2026-08-31 补记:数已经远够了 —— 28 次连续 CI 通过**,全部在含修复的
      commit 上。若失败率仍是当初记的 CI 1/5,28 连过的概率是 **0.0019,约 517
      分之一**。那一格是硬 `verdict` 不是 `pending`(`crashgate.sh:232`/`:234`),
      所以这 28 次是门禁在过、不是 pending 红被容忍。finding 已写全。
- [x] **性能轴:两格都已闭合 —— ZADD 攻下,LPUSH 被门禁挡住**(2026-08-30)。
      ...
      **LPUSH 也走完了 Phase A,结论是不攻**(同日)。它的 gate **通过**
      (与 ZADD 相反):九轮独占交错,kevy 3,140,172 vs redis8 2,907,540,
      差 232,632 对容差 83,216 —— 1.08× 是真的,是一个**窄的领先**。
      它那一格**无界增长**:3 秒窗口 append 约 3,500 万个元素。
      每命令多给几个值就能把成本拆开:335.4 / 439.0 / 640.5 / 1035.3 ns
      对应 1/2/4/8 个值 → **每命令 235.4 ns + 每元素 100.0 ns**,中间两行
      回代各差 0.8%。arena 那格只给一个值,所以 **70% 是每命令的固定开销**。
      **它自己否掉了自己的第一个候选**:ZADD 文档里按读码记下的"单值 LPUSH
      要四次键查找",实测一次查找只值 **8.1 ns**(`PING` 不碰键空间、`LLEN`
      碰一次,都用 RESP 数组),四次共 9.7%,去掉三次约省 7% —— 真实,但
      远小于一条没有数字的读码结论所暗示的。
      **Pre-Phase-B gate 判定:不攻。** 按符号只有 LTO 聚合体过 10pp;
      `debug=1` 让它能按源码行拆开,最大一行是 `sse2.rs:25` 占 19.68% ——
      而这**不是**向量化内存拷贝(第一次就是这么读错的):`--inline` 把它
      归到 `uring_reactor.rs:442`,那是 `std::hint::spin_loop()`,即 `_mm_pause`。
      **一个键的 benchmark 里,7 个 shard 在等那 1 个干活的 shard**,那五分之一
      是机器按设计空转,不是攻击面。syscall 路径占反应堆 44%,其中这台盒子的
      audit 子系统占全部样本 13%+(对四个引擎一视同仁)。再往下最大的"真工作"
      是 KevyMap 查找 2.1%。84 ns/元素与 86 ns/命令散在内联代码里,不集中。
      **随后把测量重塑两次,确认那个拒绝是关于引擎而不是关于 benchmark 的
      拓扑**:把写分散到十万个键,自旋从 20.15% 塌到 1.69%、吞吐从 2,989,978
      升到 4,295,682,真工作浮出来(SmallBytes 内联判别、`Value::List` 臂的
      `SEG_PROMOTE` 守卫、`push_front` 里的每元素 `to_vec`);但算一下就知道
      那形状也没饱和 —— 一个 shard 单独做 2,989,978,八个一起每个只做 536,960。
      于是 **1 个 shard、1 个核、100% CPU、1,945,335 ops/s**:那份剖面是**平的**
      —— **3,179 条源码行带样本,最大一条 1.92%,没有任何一条到 2%,只有 7 条到
      1%;凑到一半要 154 条、凑到九成要 1,551 条**。
      **所以那 84 ns/元素与 86 ns/命令不是由什么东西构成的**:分配、map 查找、
      原子操作、回复编码,加一条很长的尾巴,没有座位可攻。这是结论不是失败 ——
      它说剩下的成本是代码的形状本身,要动它得靠设计,不是靠指着剖面。
      `bench/PERF-DECOMP-2026-08-30-lpush-arena-cell.md`
...
## v5.4.1 — 把 mem 那一栏真的动起来(2026-08-23 属主令「追加在 5.4.x 中」→「定为 5.4.1，做好 nodefer 的线性 roadmap」)

**nodefer**:下面每一条都要做完，不许 defer、不许「先做主要的」。线性顺序，
从上往下，不跳序。做不动就说做不动并停下来问，不许自己缩水。

### 5.4.0 的诚实结账

| | kevy 常驻 | PG 常驻 | PG 数据库 |
|---|---:|---:|---:|
| KB/CSV-MB | 5,821 | 883 | 1,326 |

- **perf 是实的**：`always` 单客户端写 p99 47ms → 3.3ms、翻页 +93%
- **mem 只动了 5.1%**(5,821 → 5,527，6.6× → 6.3×)
- **disk 本来就没有差距**(1,348 vs 1,316，差 2.4%)。Axis D 排进 5.4 是设计轮的
  判断失误 —— 该先说「这一栏是平的」，而不是先算 16% 天花板、修正到 6%、再降级。

差距的分解：**表示法差距 ≈ 4.4×**(5,821 vs 1,326)是 A1/A2 该攻的；其余是
**全常驻 vs 缓冲池**(PG 只把 67% 放内存)，架构选择，优化消不掉，对放不下的
数据集已有的答案是分层。

### 线性清单（从上往下，不跳序）

- [ ] **N1 — 关掉那个符号差**。同样「先装载后声明」，完整基准两百万行**省 13.5%**，
      我的探针在每一种简化工况里都**花**内存。已实测证伪的候选(不要重跑)：
      | 候选 | 结果 |
      |---|---|
      | 索引回填的分配复用 | 带索引 +19.6%，更差 |
      | 分片数 | 两边同为盒上默认 16 |
      | 规模 | 200 万行 +3.5%，与 50 万行 +3.4% 相同 |
      | 流水线批量装载 | 逐条 +3.6% vs 批量 +3.5%，无差别 |
      **只剩一个**：装载后的**查询阶段**。基准在装载后跑 pk/idx/page/write 四种
      形状各 5000 次，探针一次都没跑。先隔离它；若它也被证伪，则我的因果模型
      缺的是**第五个我还没想到的变量**，那时必须停下来重新 decompose，不许
      在错的攻击面上继续 polish（perf-vs-foss §0）。
      **它为什么排第一**：A1 的 −23.4%(先声明)能不能到达用户手里取决于这个
      答案；`packed-rows` 默认开与否也悬在它上面。
- [x] **N1 已闭 ✅**(2026-08-23)。答案是**装载后的查询阶段**，而且方向与我的
      预期相反:加上读负载后同一路径从 **+12.4% 翻成 −3.4%**。不对称是结论 ——
      查询给通用形式加 **359 B/行**，给打包形式只加 **56**(三轮都落在 2,133，
      一字节不差)。**收益是在读的时候拿到的，不是写的时候。** 前四个候选
      (索引回填复用/分片数/规模/装载方式)全部证伪。
      见 `bench/FINDING-2026-08-23-the-gap-opens-when-the-rows-are-read.md`。
      **副产物(带到 N4)**:任何存储形式的内存测量都必须含读阶段，只测装载
      量到的是这个表示法输的那一半。

- [ ] **N3 — `used_memory` 与 RSS 脱钩(提到 N2 之前)**。降级门是
      `self.used_memory <= effective_target`(`tier_demote.rs:126`)，运维看的是 RSS。
      A1 让两者差得更远：打包降低记账值 → 引擎提前收手 → **开分层时 A1 是负的**，
      两个省内存的机制互相抵消。二选一并落地：让预算跟随 RSS，或把「预算以
      记账字节计」写进契约 + 文档 + `INFO` 里让运维看得见差额。
- [ ] **N2 — `packed-rows` 默认改为开(在 N3 之后)**。三条阻碍理由中两条已被
      实测推翻:
      | 理由 | 现状 |
      |---|---|
      | 采用路径会亏 | **推翻** —— 那句话只在「不跑查询」时成立，真实工况 −3.4% |
      | 符号差未解释 | **关闭** —— 是查询阶段，且对功能有利 |
      | 分层那栏是负的 | **仍成立** —— 所以 N3 必须先做 |
      完整基准四模式里三个是省(−13.5 / −5.1 / −12.7%)，只有分层 +4.6%。
      **N3 落地后即可默认开**；N3 未落地前不许改，否则默认开会让开了分层的
      用户看到一个可见的退步。

- [x] **N4 前置门禁 ✅ —— A2 被拦下，没写一行代码**(2026-08-23)。
      Pre-Phase-B 要求攻击面先验到双位数占比；那个 24.6% 是算术，从没测过。
      实测(50 万行，RSS 从进程外量，单变量):
      | | 无索引 | 有索引 | 索引成本 | 占比 |
      |---|---:|---:|---:|---:|
      | 短键 ~10B | 1,663 | 2,332 | **669 B/行** | **28.7%** |
      | 长键 ~36B | 1,722 | 2,509 | 787 B/行 | 31.4% |
      **索引这一项过门禁**(28.7%)。**A2 不过**：生产用的短键约 10 字节，
      换 8 字节 row-id 后每个索引每行省约 2 字节，**不到该项的 1%**。
      见 `bench/FINDING-2026-08-23-A2-attacks-under-one-percent-of-what-an-index-costs.md`。
      本轮第五次算术输给实测，也是第一次门禁跑在代码之前。

- [x] **N4' 已完成 ✅**(2026-08-23)。五种声明、单变量、加法校验闭合
      (256 + 412 = 668 对实测 670，残差 2 B ⇒ 是分解不是拟合):
      | 形状 | RSS/行 | 增量 |
      |---|---:|---:|
      | 无索引 | 1,663 | — |
      | `INDEX sku range` | 1,919 | **+256** |
      | … `VALUES name` | 2,084 | +165 |
      | `ORDERPATH dept→ts` | 2,075 | **+412** |
      | 两个索引 | 2,333 | +670 |
...
- [ ] **L1 — GitHub Actions 的 `NPM_TOKEN` 是坏的**。5.4.0 的 npm 那一环是我在
      本地用属主给的令牌手工发的；Actions 里那个 secret 未修，**下次 tag 会在
      同一处失败**。而且该令牌已出现在对话记录里，必须轮换，新值同时更新
      `.env.local` 与 Actions secret 两处。
- [ ] **L2 — 密钥保护不该依赖工作区状态**。`git stash` 掉 `.gitignore` 会连同
      保护一起 stash 走，而被保护的文件留在原地 —— 本轮真实发生过一次暴露窗口。
      办法：把 `.env*.local` 的忽略规则移到 `.git/info/exclude` 或全局
      gitignore（不随工作区变动），并加一道 precommit 检查：任何
      `git status` 里出现 `.env` 开头的文件即拒绝。
- [ ] **L3 — 约 100 个陈旧本地分支**(`worktree-agent-*`、历史 `feature/v2-*`~`v4-*`)。
      删前双零审计(已并入 + 未推 0)，一次清完。

### N4' 产出的新目标 —— 不在 5.4.1，进下一版

原清单(L1-L3、N1-N5)本身已经做完；下面三项是**分解产出的新工作**，
不是从 5.4.1 里挪走的东西。按实测尺寸排序:

- [ ] **X1 — 复合索引 412 B/行**。最大单项，是标量索引的 1.6 倍，此前无人量过。
      每行派生一个 `IndexValue::Str` 复合键，而它同样被 `tree` 和 `back` 各存一份。
- [ ] **X2 — 双份副本与那 140 B 未命名开销**。`tree` 存 `(IndexValue, Vec<u8>)`，
      `back` 存 `Vec<u8> → IndexValue` —— **值和键都存了两遍**，两个容器从不共享。
      索引需要的是 value→keys 和 key→value 两个方向，不需要各持一份完整副本。
- [ ] **X3 — A2(键换稠密 row-id)**，约 52 B/行。真实，但比上面两个都小。
      它曾被当作剩余最大的内存项(全店 24.6%)，实测是三项里的第三。

**三项都只是有了实测尺寸的 Phase A 目标，不是实现许可。**

### 不在 5.4.1（写死，不是 defer）

- **Axis D（记录格式 / disk）**：disk 已平，6% 不值一次格式变更。
- **关掉 6.6× 本身**：其中约 2.2× 是「全常驻 vs 缓冲池」的架构选择，
  A1+A2 到不了，分层才是那一段的答案。
...
- [x] **Axis A1 — 打包行**。类型 / 读路径 / 写路径 / 九处内存管理路径 /
      三个变更动词 / tick 回填 全部落地,`formgate` 挡住第四次复发。
      盒上三轮交错实测:RSS **−13.5% / −5.1% / −12.7%**(aof off /
      everysec / always),分层 **+4.6%**;装载不变;翻页贵约 7%。
      两个写在测量之前的预测都被推翻(内存 −24% 没到,装载 −8.5% 没出现)。
      按 RFC §8 第 5 步「RSS 项不动就 revert」—— 它动了,留下。
- [x] **顺序对照**(A1 最重要的一条):先声明后装载 **−23.4%**,先装载后
      声明 **+3.4%**,两者峰值相同。表示法值 −23%,回填那条路把它还回去。
      三轮逐格重复。
- [x] **`docs/packed-rows.md`** 三语 + 导航 + 摘要;**CHANGELOG 5.4.0 条目**。
- [ ] **`packed-rows` 默认值** — 属主拍板。**我的建议:5.4 维持默认关。**

      理由不是保守,是三条已测出来的事实各自都指向「还不该无条件加在所有人
      身上」:

      1. **采用场景会亏。** 对已有 keyspace 发声明(最重要的采用路径)在
         本机实测 **+55%**,盒上无索引 50 万行 **+3.4%**。省下的部分要等
         分配器归还,归还多少是平台性质。默认开等于让这条路径上的用户先
         吃一段内存上涨。
      2. **分层那一栏是负的**(+4.6%),而且原因是结构性的:降级预算以
         `used_memory` 计,打包让它降得比 RSS 快,引擎提前收手。这不是缺陷,
         但「打开一个省内存的开关之后 RSS 上升」不该是默认行为。
      3. **两组自己的测量还没对上账**:同样是「先装载后声明」,带索引的两
         百万行省 13.5%,无索引的五十万行花 3.4%。差别可能在索引回填的分配
         复用,但**没有任何测量证实**。默认值不该建立在一个未解释的符号差上。

      RFC §7「需要 5.3 行为就跑 5.3」的理由仍然成立,但它假设「这个形式在
      所有路径上都不更差」—— 第 1、2 条说明目前不是。
      **默认开的时机**:第 3 条被解释清楚,且分层那条要么修、要么写进契约。
- [ ] **`docs/rds-workloads.md` 重述** — 现发布的索引/翻页数字来自 7 月那版
      harness,当时 kevy 的计时形状**没有 `FIELDS`**,是在做更少的工作量。
      5.4 用公平口径重测的数字重写。
- [ ] **p99 = 47,264 µs ≈ `park_timeout_ms`**(主 RFC §6 点名,「无论 Axis C
      是否入选都要修」)。串行与并发两个 harness 同一签名。机制已读到底:
      `OP_AOF` 完成推进持久水位但不置 `io_work`,分片在把扣住的回复冲出去
      的那一趟 arm **之前**又 park 一次。**疑似 5.2/5.3 发出去的 15 倍回归**
      —— `48d06ae7`(08-12)把 CQE-gated 回复设为 `always` 默认,而 7 月
      同一形状是 3,097 µs。判定实验(四组、不改代码)盒上运行中。
...
**已部署 ✅**(2026-08-14,属主令「继续 autorun」)。线上验过而非只看 rsync 成功。

## playground 做全 + 四个门禁盲区 ✅(2026-08-14,属主令「把 playground 的能力设计全面」)

**merge `feature/playground-full-surface` → develop,CI 绿(31781990546),已部署并对线上验过。**

起点是属主在首页终端敲 `IDX.CREATE` 得到 `unknown command`。根因是两件事叠在一起:
wasm 只按 `["core","persist"]` 构建(把索引/全文/向量全裁了),而**站点吃的是 registry
上已发布的包**——按构造永远落后于树。任何一件单独存在都不会让首页在没编进索引的构建上
演示二级索引。

**能力面**:9 个场景 57 条命令,覆盖浏览器能达到的整个引擎(键与 TTL / 四种结构 /
二级索引的范围·过滤·排序·`COMPOSE` / 全文 `MATCH` / 向量 `KNN` / 聚合 `GROUPS` /
视图 / 表 / 键空间)。每条**先过引擎再写下来**,语法取自 `cmd_index.rs` 与 cookbook——
凭记忆写正是坏芯片的由来。另加历史翻页、整段粘贴逐行执行、按 redis-cli 规矩排版嵌套回复。

**两处有意不做,都有实测依据**:持久化开关(wasm 的 `cmd` 写入不进宿主持久化泵——写入、
关闭、重开后键没了,typed setter 还在;放开关等于请人得出"kevy 丢数据"的结论);位图场景
(浏览器答、服务器不答,`KNOWN_GAPS` 记为 "unwired on RESP dispatch",演示它等于教一条
访客自己服务器上不通的命令)。

**四个门禁盲区,形状完全一样——绿灯与"根本没检查"相容**:
- 首页的演示命令**从没被任何东西跑过** → `verify.mjs` 跑遍每个场景,任一错误回复即红
- `check_wasm_size` 的贪婪匹配吃掉整行,**每行只看得见第一个数字**(19 报绿,实际 21,藏着一个陈旧的)
- 同一道门的容差 25% **宽到能吸收量纲错误**(416 KB 的"未压缩"被当成 481 KB 的"压缩")→ 收到 10%,又翻出 4 处
- `check_cjk_punct` 的扩展名清单停在 React 改造之前,**`.tsx`/`.ts` 一个没扫** → 补上抓出 34 处,多数早于本弧

**顺手修的真缺陷**:引擎错误消息 `it {verb}es` 把 store 拼成 `storees`(两个 crate 各一份,
测试原本把错拼锁死);三语 25 处 wasm 体积数字失真(218/416/425 KB,实测 gzip 481 / raw 1442),
**在本弧动手之前就已不准**;npm 包描述与三语 FAQ 还在说它只有 KV+TTL+pubsub;站点中文 34 处半角标点。

**外壳与 tiktoken 逐属性对齐**:footer 的链接下划线曾被 `border-bottom: none` 关掉(全站唯一没有
下划线的链接就在 footer),许可行重复了 wordmark 已经说过的 GOLIA K.K. 还带年份。比对不靠眼睛——
`web/compare-lab.mjs` 与线上 tiktoken 逐属性比,**它抓到一处肉眼不可见的差异**:许可行渲染
出来一模一样却不是同一个字符串,tiktoken 用的是不换行空格。那不是硬写的字符而是变换,已按根因
移植 `phrase()`(CJK↔拉丁边界的空格变 U+00A0)。现两页 28 项计算属性 + 许可行完全一致。
该工具**不进 CI**:它会因为对方改版而红,那不是本仓库当下能处理的原因(同 `check_action_versions --online`)。

**结构改变**:`web/package.json` 依赖 `file:../crates/kevy-wasm/pkg`,`web/engine.mjs` 在每次
构建前编译引擎;CI site job 补 Rust 工具链;release skill 已改对(旧文写的是"站点加载已发布的
npm 包",并说明**版本之间**缺口现在朝另一个方向:页面可以展示已发布包还答不了的动词)。

**留给属主**:已发布的 `@goliapkg/kevy` 仍是 core-only,补齐要发新包(tag 是属主的扳机,
...
- [x] **三渠道已发并装后验证 ✅**(2026-08-13):**PyPI `kevy` 5.1.0**(OIDC 可信发布,零 token;`pip install kevy==5.1.0` 从公网装、对真服务器读写通、`mem://` 给出指路错误)· **pub.dev `flutter_kevy` 5.1.0**(产物仓 `goliajp/kevy-flutter` 生成推送;`flutter pub add` → 编译 APK → 两个 ABI 的 `libkevy_ffi.so` 在包里自报 5.1.0)· **NuGet `kevy` 5.1.0 已发但装不上,按属主决定已 unlist**(搜索 API `totalHits: 0` 确认):`IsPackable=false` 使 `Kevy.Embedded.Internal` 永不存在,而 nuspec 声明依赖它 → `NU1101`。**我的失误在没装一次** —— 读了 nuspec、看见那行依赖、理解成"两个包一起发",一次 `dotnet restore` 就能否掉;同一轮给 Python 做了装后 smoke,给 NuGet 没做。修复(`PrivateAssets="all"` + pack target 折进 DLL)已实测并进 workflow(**推送前必须先装自己的输出并跑起来**),随下个引擎版本发。三渠道介绍已上站:`docs/bindings.md` 三语,每条安装行都是先装下来跑通再写的
...
- [x] **Go 渠道已闭(2026-08-13,merge `1f0d9bb0` + `8f751e1f`)**:属主令「不独立维护 kevy-go,一律在 kevy 操作」→ 该 repo 定为**生成物**,`scripts/mirror-go-module.sh` 从 `bindings/go` 产出、验证、推送 + 打 tag,永不手改。生成靠**留下文件**而非维护 stub:嵌入式 cgo 文件加 `kevy_embedded` build tag,init 时把自己注册进 `embedded_seam.go` 的两个函数变量,其余代码经 interface 触达引擎(stub 那条路是同一表面的第二份定义,谁给一份加方法就漂)。发布的是**纯 Go 那一半**——module 走 proxy 发的是源码,几十 MB 的分平台静态库根本走不了,而纯 Go 在 Go 支持的所有平台可用而不是我们打包的三个;`mem://`/`file://` 在无 tag 构建里返回一句指明 tag 与构建位置的错误。生成器**在 kevy 树外**构建测试(树内那两条 `${SRCDIR}/../../crates` 仍能解析,只在被抽出的地方能用的模块会假绿);缺 server 二进制算失败不算 skip(全 skip 也是绿)。CI job `go-mirror` 每次推送跑,`--push X.Y.Z` 是发布步骤(已写进 release skill)。**顺手修两处覆盖回退**:ffigate 的 `go test ./...` 在 tag 出现那刻变成纯 Go 跑,把刚构建的门一行不测——已加 tag(embeddedgate harness 同);`serverBinary` 里一条开发者本机绝对路径改 `KEVY_SERVER_BIN`。**已发并真装验证**:`go get github.com/goliajp/kevy-go/v5@v5.1.0` 走默认 proxy 装成功、编译运行、校验和入 `go.sum`。发布途中撞一个真缺陷:**branch 与 tag 分两次 push 之间有窗口**,proxy/sumdb 若在窗口内抓取会记 `unknown revision` 并缓存远比窗口长的时间——症状自相矛盾(`@latest` 与 `@v/list` 200 且元数据正确,而 `.info`/`.mod`/`.zip` 全 404),`GOPROXY=direct GOSUMDB=off` 全程可用才把「产物坏了」和「缓存陈旧」分开;且两者是**串联**(sumdb 要从 proxy 拿 zip 才能算校验和,故 proxy 404 时 go get 报的是 proxy 的错却挂着 sumdb 的 URL)。脚本已改 `--atomic` 单次 ref 更新。finding=`bench/FINDING-2026-08-13-go-generated-mirror.md`
- [x] **maven 代码侧全部就绪 ✅**(2026-08-13):pom 补齐 Central 必需的 `licenses`/`developers`/`scm` + `-Prelease` profile 下的 sources/javadoc jar 与 gpg loopback 签名;`scripts/publish-maven-central.sh` 移植自 sentori,**继承它三条实证坑**(Portal 收 **POST 的 zip bundle** 而非 PUT 仓库,`distributionManagement` 指向上传端点看着对但永远不工作 / 签名公钥必须在**保留 UID** 的 keyserver 上——keys.openpgp.org 在邮箱验证前会剥掉 UID,而 GnuPG 拒收无 UID 公钥,脚本用「只含陌生人能拿到的公钥」的 keyring 验签,与 Central 的做法一致 / **`PUBLISHED` 只是 portal 的说法,最后一步去问 repo1**);差异改写:sentori 是 Gradle、kevy 是 Maven → `mvn -Prelease deploy` 到 `file://` staging repo。`.github/workflows/maven-central.yml` 手动触发、默认只验证不发布。语法与无凭据守卫已实证
- [x] **maven 已发布 ✅**(2026-08-13):四个凭据已提到 `goliajp` org 级(`CENTRAL_USERNAME`/`CENTRAL_PASSWORD`/`SIGNING_KEY`/`SIGNING_PASSWORD`,visibility=selected 含 kevy+sentori),`jp.golia:kevy:5.1.0` **在 repo1 在册**(不看 Portal 的 `PUBLISHED`,看 repo1 实际服务的文件)。**找签名密钥的姿势值得记**:本地 keyring 里那把是 smix 的(`FBD802632CFAD78B`),而 Central 上 jp.golia 的产物是 `22BD3D63FE94A270` 签的——**问 repo1「已发布的 .pom.asc 是谁签的」才定了案**,不是翻本机。另两个坑:`gh secret set --body -` 会把**字面的一个短横**存进去(不是读 stdin 的标记),它成功得很响而在 gpg 报「no valid OpenPGP data」时才炸;「有没有配 passphrase」证明不了什么(Gradle 文件里那个键存在但值为空,看着像配好了)——**拿它签一次**才是判据
- [ ] **属主收尾(2026-08-14 属主令「你不用管了」——不再由我跟踪,不作为开放项复述)**:sentori 上那四个**仓库级** secret 仍在,而仓库级会遮蔽同名的组织级 —— 不删则 sentori 继续用自己的副本,正是这次提升要终结的漂移。**等一次 sentori publish 在 org 副本上跑绿之后再删**(遮蔽规则的意义就是让你先验证再撤退路):`gh secret delete <NAME> -R goliajp/sentori` ×4

命名与结构两类**已定并落地**(ts 名 / SPM 根 manifest / Go 生成镜像),maven 已发布。**只剩 NuGet / PyPI / pub.dev 三个渠道的凭据**要属主给;给了之后我这边可以直接推进。
