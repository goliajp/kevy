...
- [ ] **maven 只差把凭据提到 org 级**(属主已确认这是正确做法):`goliajp` org 现在**零个 secret**(实测 `total_count: 0`),那四个只在 `goliajp/sentori` 仓库上。**GitHub 从设计上不返回 secret 明文**——所以提升只能由持有原件的人做,跟我的权限无关。提上去之后 kevy 无需改动即可跑该 workflow
