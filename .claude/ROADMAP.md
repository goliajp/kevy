...
- [ ] **属主收尾**:sentori 上那四个**仓库级** secret 仍在,而仓库级会遮蔽同名的组织级 —— 不删则 sentori 继续用自己的副本,正是这次提升要终结的漂移。**等一次 sentori publish 在 org 副本上跑绿之后再删**(遮蔽规则的意义就是让你先验证再撤退路):`gh secret delete <NAME> -R goliajp/sentori` ×4
