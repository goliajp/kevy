...
## 7. Resolution(2026-08-13,属主授权「你来定」后拍板)

四个点全部收口。①③④ 的判定与 §6 的建议一致——建议是带着实测写的,
复核后没有推翻它的理由;②维持不做。

**① bar 重锚 = B-1 双线,终值如下。** 章程的「89×80%」是在测量之前
写下的句子(分母把永久具名拒的那 52 个探针也吃了进去),按它设线只会
逼出一个假绿。改成两条各自可证伪的线:

- **线①(能力)**:函数子集 `subset-foldable ≥ 82%`。实测 82.5%
  (395/479),按 perfgate 的 ratchet 哲学**只升不降**。分母是"这一弧
  声明要服务的那部分",不是 89 个文件。
- **线②(诚实)**:全 89 探针 **`wrong == 0`** —— 硬线,不是 ratchet。
  按名拒绝是诚实的,答错不是。实测 89/89 文件零 wrong。

两条线都已在 `bench/funcgate.sh` 里,本次只是把"pending 拍板"的措辞
换成终值,并把线②的地位写明为硬线。

**③ regexp = fork spg + 改壳(已实施)。** `kevy-scalar/src/regex_engine/`
1260 行,自 spg 的 ERE 匹配器 fork。理由与 `.claude/CLAUDE.md` 的
LOC-WAIVER 第二类同源:拆分/重写一份已测的引擎核心是注 bug 的高发面,
而换来的可读性收益为零。零依赖铁律不变(纯 Rust、无 crates.io 依赖)。

**④ md5 = 自研(已实施)。** `kevy-scalar/src/md5.rs`。引一个 crate 会
破零依赖铁律,而 md5 是定死的算法、无演进面。

**② 写时计算列 = 维持不做。** 它要动引擎写路径,而 V1 的落点被 Law 3
钉死在 sql 面(引擎零改动)。V4 之后另立。
