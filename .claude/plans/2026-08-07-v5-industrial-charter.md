...
| # | 件 | 选项与我的建议 |
|---|---|---|
| P1 | **kevy-alloc 默认开关** | ON(内存赢 10%,集合角输 10-14%,R4a 证据:SME 事故全在读聚合非集合写)/ OFF(反之)。建议 **ON**,集合角 floor 改按"对上一发布版不退",V4 顺带尝试 B' 收窄。**终判 2026-08-09:opt-in(A 案)**——原拍 ON 的前提「集合角可收复」被 V4 测量链推翻:hset 已由 value-inline 收复(−0.2%),sadd/zadd 残税被证同源 = 饱和 owner 快路径每调用成本(~1.7×/call,无主导座位),B' 证伪、C 案成本效益差。价值面(容量/碎片 2.16×)以 opt-in + docs/alloc.md 指南交付。全链见 fastpath-residue RFC Resolution |
...
## 六、拍板已定(2026-08-08,属主授权「你来定」,由研究者拍板)

属主 2026-08-08 明确授权:「你来定吧,这是你做的研究,你更清楚应该怎么做」。
据此把累积的 P1-P4 + V1 三件全部拍死如下(不含 tag/publish 等不可逆
发版动作,那些仍留待明确指令):

| 件 | 定 | 依据 |
|---|---|---|
| P1 alloc 默认 | **ON** | 组合态实测(平衡轮)disk/容量/stable 零代价,代价=且仅=集合角;R4a 事故全在读聚合非集合写 |
| P2 集合角 floor | **改 vs 上一发布 ratchet** | 工业口径对自己负责,不对假想 glibc-off 对照负责 |
| P3 mailrs 语料 | **defer** | V1-V3 不需要;合成语料 + 定性口径已足;真语料脱敏等属主有暇再给 |
| P4 版本时点 | **rc 一档**(V1-V3 完出 5.0.0-rc,渠道全流程彩排;V6 再 5.0.0) | 不可逆 publish 前留缓冲 |
| V1① funcgate bar | **B-1 双线**:子集可折叠 **≥80%**(现 74.3%,ratchet 爬)+ 全 89 探针 **100% 可分类**(零静默)+ **wrong==0** 硬线 | 89×80% 逐探针天花板 ~50% 不可达;子集 80% 是真能达且有意义的线 |
| V1③ regexp 路线 | **fork spg 的 ERE 引擎** | 同厂产线代码、恰满足 no_std/零依赖/纯 Rust、探针语料即验收 oracle;否则子集 80% 够不着(13 探针挡在③) |
| V1④ md5 | **做**(已落 `7bcd65c8`) | 标准算法、RFC 1321 向量验证、补齐 R4a hash 项 |
| AOF-offload ② | 背压=**阻塞**(=今日行为,仅真慢盘时触发) | 与今日等价;正常态零阻塞;reactor 接线走 off-by-default flag 单独一轮 |

*P1-P4 + V1 三件已由研究者据授权拍定;V0 已 push(2026-08-08)。*
