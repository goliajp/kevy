...
**再分解,才看见。** `perf record` 抓查询阶段的符号剖面:**87% self-time 在
libc 分配器**(`cfree` 26% + 一串 malloc 内部)。短语路径是**分配受限**,不是
交集受限——这既解释了那次倒退(把哈希换成归并,一个分配都没减,还多加了一个
`next` Vec),也指出了真源头:`Positions::get` 每次调用把 blob 解码进一个新
`Vec`,head+head 约 40 万候选 × 2 词 ≈ **每查询 80 万次分配**。
