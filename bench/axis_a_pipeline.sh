#!/bin/bash
# Axis A: deep pipelining sweep — -P 1 / 4 / 16 / 64 / 256.
# Hypothesis: io_uring multishot recv + E14 enter-skip + L1 writev
# means kevy's per-cmd kernel cost stays flat as -P grows, while
# valkey's epoll wakeup-per-batch + per-cmd dispatch cost grows.
# Predict: kevy ≥120 % of valkey at -P 64+.
#
# Output:
#   stdout : per-row TSV  axis=pipeline server P op rps
#   /tmp/axis_a.tsv : same
set -u
...
# Median of $RUNS for one (op, P, server).
bench_one() {
  local op_uc=$1 op_lc=$2 P=$3
  local -a samples=()
  for _ in $(seq 1 "$RUNS"); do
    local out
    out=$(taskset -c "$CLI" "$REDIS_BENCH" -h 127.0.0.1 -p "$PORT" \
      -t "$op_lc" -n "$N" -c 50 -P "$P" -q 2>&1 | tr '\r' '\n')
    local v
...
run_server() {
  ...
  for P in 1 4 16 64 256; do
    for op in SET GET; do
      ...
      local rps; rps=$(bench_one "$op" "$lc" "$P")
      printf "%s\t%s\t%d\t%s\t%s\n" "pipeline" "$srv" "$P" "$op" "$rps" \
        | tee -a "$OUT"
    done
  done
}
...
data = defaultdict(dict)  # data[(P, op)][srv] = rps
servers = []
for ln in open(sys.argv[1]):
    parts = ln.strip().split('\t')
    if len(parts) != 5: continue
    _, srv, P, op, rps = parts
    if srv not in servers: servers.append(srv)
    data[(int(P), op)][srv] = float(rps)
print(f"| -P | op | {' | '.join(servers)} | kevy% best | verdict |")
print("|---|---|" + "|".join("---" for _ in servers) + "|---|---|")
for (P, op), row in sorted(data.items()):
    ...
    print(f"| {P} | {op} | {' | '.join(cells)} | {pct:.0f}% | {verdict} |")
PY
