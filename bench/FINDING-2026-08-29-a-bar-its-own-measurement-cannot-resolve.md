...
# 补:把所有门扫一遍之后,cell D 是唯一的例外

修完 tailgate 之后回头找同一个形状 —— **哪些门测了一个会波动的量,然后用
单次绝对读数去判它**。答案是:**没有别的。**
...
| 门 | 噪声指标怎么判 |
|---|---|
| `perfgate` | 参照与候选**交替测量**(顺序还随机化),盒子漂移相互抵消而不是替它判断;失败角度加大 n **重测**,并在输出里标注 `[retested, n=…]` |
| `tiergate` L15 | 同一窗口测两次(tiering 关 / 开),判**比值** ≤ 3× + 20 tick 余量 —— 历史值是 300–500× |
| `crashgate` / `availgate` / `repligate` | 判的是确定性契约,不是时序量 |
| **`tailgate` cell D** | **绝对线 100 ms,单次样本** |

`perfgate` 那段交替测量的注释里记着它自己的来历:2026-07-12,**同一份代码**
在基线说别的数时量到 24.3M —— 于是交替成了默认。

所以本文给 tailgate 加的两件事(污染探测丢弃重取、越线重取确认)**不是发明,
是把一道门拉回其余门早就在做的事**。

而扫查真正的产出,是把剩下那个开着的问题说准了:**其余每一个噪声门判的都是
比值或配对差,只有它判绝对值。** 那条线该不该继续是绝对值,是属主的判断 ——
这句话是让那个选择可说的部分。
