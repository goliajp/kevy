...
run3med() {
  local op=$1 c=$2 n=$3 extra="${4:-}"
  declare -a s=()
  for _ in 1 2 3; do
    v=$(timeout 60 taskset -c 10-13 "$RB" -h 127.0.0.1 -p $PORT -t "${op,,}" -c $c -P 1 -n $n $extra -q 2>&1 | tr '\r' '\n' | grep -E "^${op}: [0-9]" | tail -1 | awk '{print $2}')
    s+=("${v:-0}")
  done
  printf '%s\n' "${s[@]}" | sort -n | awk 'NR==2{print}'
}

echo "=== F1 bench: overwrite-SET single-probe via raw_entry_mut ==="
echo "Axis C — random keys (deco bench shape):"
for srv in kevy valkey; do
  start_${srv}
  r100k=$(run3med SET 50 500000 "-r 100000")
  r1M=$(run3med   SET 50 500000 "-r 1000000")
  r10M=$(run3med  SET 50 500000 "-r 10000000")
  printf "%s SET -r100k=%-10s -r1M=%-10s -r10M=%-10s\n" "$srv" "$r100k" "$r1M" "$r10M"
done
echo "Sanity (post-F1, no regression expected):"
for srv in kevy valkey; do
  start_${srv}
  ...
  printf "%s c1-P1 SET=%-8s GET=%-8s c50-P1 SET=%-8s GET=%-8s\n" "$srv" "$c1s" "$c1g" "$c50s" "$c50g"
done
kill_all
