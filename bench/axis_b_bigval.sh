...
# Hypothesis: L1 Value::ArcBulk + writev iovec means kevy GET on
# values > BULK_THRESHOLD (64 B) skips the per-GET memcpy of the
# value into per-conn output. valkey has tryAvoidBulkStrCopyToReply
# but its robj chain + jemalloc fragmentation means it still pays
# more allocator + cache work per big GET.
# Predict: kevy ≥120 % of valkey starting at -d 4K.
set -u
...
bench_one() {
  local op_uc=$1 op_lc=$2 D=$3 N=$4
  local -a samples=()
  for _ in $(seq 1 "$RUNS"); do
    local out
    out=$(taskset -c "$CLI" "$REDIS_BENCH" -h 127.0.0.1 -p "$PORT" \
      -t "$op_lc" -n "$N" -c 50 -P 1 -d "$D" -q 2>&1 | tr '\r' '\n')
    local v
...
# (-d, -n) pairs. Smaller n for big values to keep wall-clock reasonable.
declare -a SCENS=(
  "64 600000"
  "256 600000"
  "1024 400000"
...
run_server() {
  ...
  for s in "${SCENS[@]}"; do
    ...
    for op in SET GET; do
      ...
      local rps; rps=$(bench_one "$op" "$lc" "$D" "$N")
      printf "%s\t%s\t%d\t%s\t%s\n" "bigval" "$srv" "$D" "$op" "$rps" \
        | tee -a "$OUT"
    done
  done
...
for ln in open(sys.argv[1]):
    ...
    _, srv, D, op, rps = parts
    if srv not in servers: servers.append(srv)
    data[(int(D), op)][srv] = float(rps)
print(f"| -d (B) | op | {' | '.join(servers)} | kevy% best | verdict |")
print("|---|---|" + "|".join("---" for _ in servers) + "|---|---|")
for (D, op), row in sorted(data.items()):
    ...
    print(f"| {D} | {op} | {' | '.join(cells)} | {pct:.0f}% | {verdict} |")
PY
