...
## 第五幕:第三个"wedge"根本不是 wedge —— 我读错了自己的证据(2026-07-19)

第四幕结尾我留下一个"仍然开着的第三个 wedge":node-redis 在 FLUSHALL 之后的
SET 上永久挂死,并据此建了 `bench/xshardwedge.sh`。**那个判断是错的,harness 已删。**

### 错在哪

判据是"node 输出的最后一行是 `step: SET`"。但 `step("SET")` 是那个脚本里
**最后一个** step 标记 —— 它后面还有 GET / INCRBY / PTTL / 哈希 / 列表 / 有序集 /
pub/sub / IDX.LIST 一整段,全都没有标记。所以"最后一行是 step: SET"的真实含义是
**"卡在 SET 之后的任何地方"**,而我把它读成了"卡在 SET"。

推翻它的是抓包:线缆上早已跑到 `PUBLISH room hi`,而且服务端答的是 `:0`
(零订阅者),subscribe 的确认在 130µs **之后**才发出。

### 真相

脚本写的是 `new Promise((r) => sub.subscribe("room", cb))` —— **subscribe 没有被
await**,紧接着就从另一条连接 publish。这在任何服务器上都是无保证的竞态:消息只
投递给 PUBLISH 那一刻已注册的订阅者。

对照实验(每格 40 轮):

| | 丢消息 |
|---|---|
| valkey 9.1,未 await | 0/40 |
| kevy 16 shards,**已 await** | **0/40** |
| kevy 16 shards,未 await | 2/40 |

⟹ **kevy 没有违反 Redis 契约**:`subscribe()` 一旦兑现,跨连接的 publish 必达。
输的只是那个无保证的窗口 —— 单线程的 valkey 因为"到达顺序即处理顺序"恰好不会输,
thread-per-core 且两条连接落在不同 shard 时会。**这是一条值得记录的行为差异,不是
一个 wedge。** 修复落在 clientgate 脚本(await 订阅 + 给后半段补上 step 标记,
20/20 绿),不在服务端。

### 之前那些"证据"现在怎么解释

- **矩阵**(uring 16sh 6/6、4sh 1/6、1sh 0/6、epoll 16sh 0/6):数据是真的,含义变了。
  它量的是**跨 shard 订阅可见性窗口**随 shard 数放大,不是跨核消息搁浅。单 shard
  为 0 是因为不需要传播;epoll 为 0 是因为它的内联 flush 让 subscribe 往返更快。
- **全部线程 0% CPU 睡眠**:与 park 每 50ms 超时唤醒完全一致 —— 循环一直在跑,
  没有任何东西被搁浅。
- **`qbuf=0 oll=0 cmd=NULL` + 四个内核队列全 0**:两端都排空了,因为**服务端确实
  答完了它被问的每一件事**。缺的那条消息从来不欠任何人。
- **STALL 转储一条不打**:不是"转储没跑"(心跳证明它跑了),是**真的没有连接处于
  不可推进状态**。这条负面结果当时就该让我怀疑整个前提,我却又追了两轮。

### 教训

1. **"最后一行"不等于"卡住的那一行"** —— 只有当每个阶段都有标记时才等价。步进
   标记本来就是为此加的(脚本注释白纸黑字写着"name the exact command that never
   resolves"),而它只覆盖到第三个命令就断了,于是整整一天的调查建立在一个未经检验
   的读法上。
2. **负面结果要当证据用,不是当噪音跳过。** STALL 转储什么都没打、四个队列全 0 ——
   这两条各自都在说"服务端没有欠任何东西",我却把它们当成"还没找到",继续在服务端
   里找。
3. **抓包比读源码先到。** 三轮源码假设(seq 退休、backlog dirty 位、arm 队列)全部
   落空;一次 tcpdump 直接给出答案。挂死类问题应该**先看线缆**再看代码。
4. **对照组要早。** "同一脚本打 valkey 会怎样"这个实验成本极低、区分度极高,我却
   拖到最后才做。任何"我们的服务器有 bug"的判断,在有现成对照实现时都应该先跑对照。

`8368bb09`(backlog dirty 位竞态)保留:那条竞态按代码推演成立,提交信息也已写明它
不是任何症状的修复。`bench/killgate.sh` 与 uring 的两个真修复(`8d8f20e9` /
`17c7062f`)不受本节影响 —— 它们各有独立的实测证据。
