...
#   CAPACITY_ONLY   D1 | B6 — run one phase; writes its own results file
#                   (`-only-<phase>`) so tiergate is never handed a file
#                   with lines silently missing.
#   B6_KEYS_OVERRIDE / B6_VAL_OVERRIDE / B6_BUDGET_OVERRIDE — resize the
#                   B6 phase. Shrinking the budget against a fixed dataset
#                   raises the data:RAM ratio, which is how you find where
#                   the envelope ENDS rather than re-confirming the 10x it
#                   was sized for.
#
...
# B6 sizing overrides — for searching where the envelope actually ends
# rather than re-confirming the ratio it was designed for. The 2026-07-25
# full run passed at exactly 10.0x because 5M x 4KiB on 2GB IS 10x; that
# says the contract holds there, not where it breaks. Shrink the budget
# against a fixed dataset and the ratio climbs.
B6_KEYS=${B6_KEYS_OVERRIDE:-$B6_KEYS}
B6_VAL=${B6_VAL_OVERRIDE:-$B6_VAL}
B6_BUDGET=${B6_BUDGET_OVERRIDE:-$B6_BUDGET}

# A sweep needs one phase, not both. A partial run writes its own results
# file: tiergate consumes .capacity-envelope-results and must never be
# handed a file with lines silently missing.
ONLY=${CAPACITY_ONLY:-}
case "$ONLY" in ''|D1|B6) ;; *) refuse "CAPACITY_ONLY must be D1|B6" ;; esac
[ -n "$ONLY" ] && RESULTS="$RESULTS-only-$ONLY"
...
if [ "$ONLY" = B6 ]; then note "phase D1: skipped (CAPACITY_ONLY=B6)"; else
...
if [ "$ONLY" = D1 ]; then note "phase B6: skipped (CAPACITY_ONLY=D1)"; else
...
# any failure there already set FAILED; fold it into the lines they own.
{
  ...
  echo "SCALE=$SCALE${ONLY:+ (phase $ONLY only)}"
  if [ "$ONLY" != B6 ]; then
    L12=$(combine "$V_C4" "$V_C5" "$V_HYD"); L13=$V_HYD; L14=$V_D4
    if [ "$FAILED" = 1 ] && [ "$ENFORCE" = 1 ]; then L12=FAIL; L13=FAIL; fi
