...
一个纯 Rust、零依赖、兼容 Redis 的键值存储。可以作为独立服务器，
也可以作为进程内库，或两者并用——每种形态都讲 RESP2，所以
`redis-cli` 和所有 Redis 客户端库无需任何修改即可工作。
...
的 CDC feed（内建 outbox），以及带校验和验证的迁移工具链——全部
derived-by-construction（在写路径里同步维护、永不漂移、可从数据
重建）。3.x 主线补上了机器面——自描述的 verb 契约（`COMMAND DOCS`、
生成的参考文档、`kevy-mcp` 里的官方 MCP 服务器）——以及可用性这条
...
|---|---:|---:|---:|
| `GET -c 50 -P 16` | 7.37 M/s | 3.29 M/s | **2.24×** |
| `SET -c 50 -P 16` | 6.97 M/s | 1.70 M/s | **4.10×** |
...
Stable Rust 1.97.0，Rust 2024 edition。在 Linux（`x86_64`、`aarch64`）
