...
4.0はさらに容量の上限を一段引き上げます——capacity arcです。
**透過的ティアリング**はストアにRAM予算を与えます（コールドな値は
使い捨てのディスク上value logへ退避し、アクセス時に戻ります。
コールドキーでもすべてのコマンドの意味は厳密に不変で、AOFの永続化
コントラクトはバイト単位で無変更——RAMがキー数の上限を、ディスクが
データ量の上限を決めます）。**TABLEレイヤー**は宣言時に、型付き
カラム・セカンダリインデックス・複合`ORDER BY`パス、さらには
`kevy-sql`によるPG/MySQLスキーマファイルまでを名前付きインデックスへ
コンパイルし、index-onlyクエリは全行がコールドでもRAMだけで答えます。
[docs/tiering.md](docs/tiering.md)と
[docs/tables.md](docs/tables.md)（英語）を参照してください。
