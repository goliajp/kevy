...
| 組み込み `get`(ヒット) | 9.0 M/s | — | (in-process の Redis は無い) |

サービング面は redis-stack 7.4.7(RediSearch)と同一シード・
同一コーパスで recall を揃えて比較([`bench/PERF-LEDGER.md`](bench/PERF-LEDGER.md)):

| クエリ種別 | kevy | RediSearch | 判定 |
|---|---:|---:|---|
| 全文検索(BM25 top-10) | 330 qps | 273 qps | **+21% qps**、p95 同等 |
