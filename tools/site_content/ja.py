    ...
    "blocks": [
        ...
        },
        {
            ...
            "body": (
                ...
                "による反復ではなく、<code>ZRANK</code> は O(N) です。"
    ...
    "blocks": [
        ...
        },
        {
            ...
            "body": (
                ...
                "<code>ZRANK</code> は O(N) で、シャードをまたぐ "
            ...
            "items": [
                {
                    ...
                    "a": "ワイヤの上では、なります。RESP2 と RESP3、184 個のコマンドに対応し、クライアントライブラリは違いに気づきません。挙動もおおむね同じですが、その例外こそが要点です。<code>SCAN</code> はカーソルによる反復ではありません。1 回の呼び出しでキースペース全体を走査してカーソル 0 を返すので、いつもの SCAN ループは 1 往復で終わります。<code>ZRANK</code> は O(N) です。ソート済みセットに順位の索引を持たないからです。シャードをまたぐ <code>RENAME</code> は原子的ではありません。<a href=\"~/docs/commands/\">184 個すべてのコマンドに、本当の差異と本当のコストを併記してあります</a>。Redis の文書から書き写したものではなく、実装から読み出したものです。",
                },
                {
                    "q": "マシンが落ちたら、どうなりますか",
