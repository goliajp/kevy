...
```xml
<dependency>                                     <!-- Java -->
  <groupId>jp.golia</groupId><artifactId>kevy</artifactId><version>5.2.0</version>
</dependency>
```
...
The same `GET -c 50 -P 16` face, four engines on one box — kevy at 7.37 M/s against each (median-of-5; method and per-engine cycle
accounting in
